...
`antwika::gfx` pulls in GLM and `antwika::log`, so a `ttf` that names `gfx::Bitmap` for one struct of three fields has taken all of that on, and every future caller pays it.
...
`src/libs/ttf/tests/SyntheticFont.cpp` builds a whole, valid, four-glyph TrueType font in memory: an offset table, `cmap`, `glyf`, `head`, `hhea`, `hmtx`, `kern`, `loca` and `maxp`, written byte by byte.
