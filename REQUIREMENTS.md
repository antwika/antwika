...
- A window's close request must reach the engine only as replayable input through `ITickEventSource`, never by short-circuiting the tick loop.
...
- Live input must reach the engine only through `ITickEventSource`, so a recorded interactive session replays to the same state; no second entry point may exist.
