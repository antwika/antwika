...
| `ITickEventSource.hpp` | `ITickEventSource` | `eventsFor(tick)` — the single seam between live and replayed runs. |
...
| `WindowInputSource.hpp` | `WindowInputSource` | An `ITickEventSource` that turns a window close request into `engine.stop`. |
...
`ITickEventSource` says what `eventsFor(tick)` does, and leaves the word replay to the library that has a file.
...
[`app`](app.md)'s `FramePacedSource` wraps an `ITickEventSource` instead: it draws the extra frames in the gap before a tick's events are read, then returns what the inner source returned, unchanged.
