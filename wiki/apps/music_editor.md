...
Pausing stops the musical clock rather than the notes already handed on: the mixer works about 150 ms ahead, so a note decided inside that lookahead still begins after the pause lands -- that sliver is the lookahead doing its job, not a pause being missed.
...
And it plays along with itself: the very characters of each note are lit from the very tick its audio window begins, for as long as the note is active, so a running score reads as the rhythm it is making.
