...
`WindowCloseSource` is an `IReplaySource` decorator that appends `engine.stop` once the window has gone, so a close lands in a `--record` file like anything else.
