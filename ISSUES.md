...
## 1. "One dimensional" vs. a 2D showcase (Sudoku) — confirmed
...
**Confirmed reading**: "one dimensional" is the library's own data
model and API — a flat, index-addressed `std::vector<Domain>` with
zero built-in notion of rows, columns, or neighbors. All spatial
structure (Sudoku's rows/columns/boxes, or a classic tile strip's
left/right adjacency) is expressed entirely by the *caller* as
constraints over flat indices. `PLAN_WFC.md` §3.1 and §7 cover this,
including a literal 1D demo (`OneDimensionalWfcTest`) kept in the
library's own test suite so "one dimensional" is demonstrated
directly, not only inferred from Sudoku's index math. No plan changes
needed — this is exactly what was already written.

## 2. `apps/sudoku` skipping the engine/replay stack — confirmed
...
**Confirmed**: this is fine — no tick-loop wrapper needed. No plan
changes required.
...
- **Could have**: `apps/sudoku` is a WFC showcase app that
  ...
  (confirmed, item 2 above) — worth a line clarifying that not every
  app is required to sit on the tick-loop/replay stack.
...
## 5. Plan file location, naming, and disposition — confirmed
...
**Confirmed**: root-level naming stays as is, and — unlike the ECS
precedent, where deletion was left as an option weighed later —
deletion of both files is now definitive, not conditional: they exist
purely as implementation guidance and must be removed once the
implementation and its `blog/` write-up are both complete
(`PLAN_WFC.md`'s new opening note, `PLAN_WFC_CHECKLIST.md` §12's final
item).
