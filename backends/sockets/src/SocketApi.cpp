...
namespace antwika::network::sockets
{
...
#ifdef _WIN32
        /**
         * @brief Winsock's process-wide start-up and tear-down, paired.
         */
        class Runtime final
        {
        public:
    ...
    void allowReuse(Handle handle)
    {
        ...
        // A listener that cannot say so still binds.
        // The only cost is a lingering port refused for a minute.
        (void)::setsockopt(
            handle,
            SOL_SOCKET,
            SO_REUSEADDR,
