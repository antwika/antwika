...
using antwika::music_editor::configFromJson;
using antwika::music_editor::configToJson;
using antwika::music_editor::kConfigFormatVersion;
using antwika::music_editor::kConfigMagic;
using antwika::music_editor::loadConfigFileOrDefaults;
using antwika::music_editor::readConfig;
using antwika::music_editor::standardConfigMigrations;
using antwika::music_editor::writeConfig;
...
namespace
{
    ...
    [[nodiscard]] MusicEditorConfig retuned()
    {
    ...
    void expectEqual(
        const MusicEditorConfig &decoded,
        const MusicEditorConfig &expected)
    {
        EXPECT_EQ(decoded.tickIntervalMs, expected.tickIntervalMs);
    }

    void expectDefaults(const MusicEditorConfig &decoded)
    {
        EXPECT_EQ(decoded.tickIntervalMs, MusicEditorConfig{}.tickIntervalMs);
    }
    ...
    protected:
        ...
        std::filesystem::path directory{
            ...
            / ("antwika-music_editor-config." + std::to_string(::getpid()))};
    };
} // namespace

TEST_F(ConfigFileTest, ADocumentStatesItsFormatAndItsVersion)
{
    const auto encoded = configToJson(MusicEditorConfig{});
...
{
    auto document = configToJson(MusicEditorConfig{});
