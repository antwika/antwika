...
TEST(WaveImageCacheTest, Refresh_RendersOneImagePerWaveformLine)
{
    Score score;
    score.read(
...
TEST(WaveImageCacheTest, Refresh_AQuietRefreshAnswersTheSameImages)
{
    Score score;
...
TEST(WaveImageCacheTest, Refresh_AnEditRerendersTheLineItChanged)
{
    Score score;
    score.read("$: drum.n(\"0\").hold(10).rel(10).waveform()\n");
...
TEST(WaveImageCacheTest, Refresh_TheSpeedIsPartOfTheKey)
{
    Score score;
    score.read(
...
TEST(WaveImageCacheTest, Refresh_AVanishedWaveDropsItsImage)
{
    Score score;
    score.read("$: drum.n(\"0\").hold(10).rel(10).waveform()\n");
...
TEST(WaveImageCacheTest, Refresh_ReadsASpeedPastTheTableAsLast)
{
    Score score;
    score.read(
        "$: drum.n(\"0 ~\").hold(400).rel(10).waveform()\n");
