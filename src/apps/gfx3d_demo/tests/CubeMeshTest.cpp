...
namespace
{
    ...
    /// A normal as something a std::set can order.
...
TEST(CubeMeshTest, CubeMesh_HasItsOwnQuadPerFace)
{
    ...
    // Four vertices per face, not eight shared corners.
    // A corner joins three faces that disagree about its normal.
...
TEST(CubeMeshTest, CubeMesh_WindsEveryTriangleOutwards)
{
    ...
    // The check that actually matters, and one only a picture shows.
    // A backend culling back faces shows a cube wound inside out.
    // That reads as a lighting bug rather than a winding one.
         ...
         ++triangle)
    {
        ...
        EXPECT_GT(glm::dot(face, outward), 0.0F)
            << "triangle " << triangle << " is wound inwards";
    }
}
...
TEST(CubeMeshTest, CubeMesh_AgreesWithTheNormalItDeclares)
{
         ...
         ++triangle)
    {
        ...
        EXPECT_FLOAT_EQ(glm::dot(face, first.normal), 1.0F)
            << "triangle " << triangle << " faces away from its normal";
    }
}
