...
namespace antwika::atlas_editor
{
    ...
    namespace
    {
            ...
            const EditorState &state)
        {
            ...
            dump.showPivot = state.pivotVisible();
            dump.showPointerBorder = state.pointerBorderVisible();
            dump.under = state.hovered();
            dump.marked = state.selection();
            dump.gesture = state.currentGesture();
            dump.preview = state.preview();
    ...
    EditorSnapshotStore::EditorSnapshotStore(
        ...
        : console::IJsonSnapshotStore<std::runtime_error>(
        ...
        const std::string &path, const nlohmann::json &dumped)
    {
        ...
        state.restore(SessionRestore{ // GCOVR_EXCL_LINE
            ...
            .showPivot = dump.showPivot,
            .showPointerBorder = dump.showPointerBorder,
            .under = dump.under,
            .marked = dump.marked,
