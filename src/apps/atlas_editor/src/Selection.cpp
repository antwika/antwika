...
namespace antwika::atlas_editor
{
    ...
    namespace
    {
        // One past a selection's last pixel, on either axis.
        // Signed throughout, because an origin may be off the sheet.
    ...
    Selection selectionBetween(const Pixel from, const Pixel to) noexcept
    {
        ...
        // Both corners are inside, so the span is the difference plus one.
        // A press and a release on one pixel select that pixel.
        return Selection{
            .origin = {.x = left, .y = top},
            .size = {
        ...
        const Selection &selection, const Size sheet) noexcept
    {
        ...
        // Rather than an empty rectangle every caller would test for.
