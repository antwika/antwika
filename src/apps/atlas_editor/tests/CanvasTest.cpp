...
TEST(CanvasTest, Construct_ThrowsOnAnIncompleteBitmap)
{
    const Bitmap wrong{
...
TEST(CanvasTest, Construct_TakesTheBitmapAsItIs)
{
    const Bitmap image{
...
TEST(CanvasTest, Set_WritesAPixelAndCountsTheChange)
{
    ...
    // That is what proves the offset rather than the write.
    EXPECT_EQ(canvas.at(Pixel{.x = 0, .y = 0}), kClear);
}
