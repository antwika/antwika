...
TEST(CanvasViewTest, Equality_ComparesThePanAndTheZoomLevel)
{
...
TEST(PixelTest, Equality_ComparesBothCoordinates)
{
...
TEST(CanvasViewTest, CentredView_PutsASheetWiderThanTheCanvasAtANegativePan)
{
    ...
    // Four screen pixels per image pixel, so the sheet is 160 wide.
...
TEST(CanvasViewTest, PixelAt_FloorsRatherThanTruncates)
{
    ...
    // One screen pixel left of the sheet is pixel -1, not a second 0.
    EXPECT_EQ(
        pixelAt(view, Point{.x = 9, .y = 8}),
        (Pixel{.x = -1, .y = -1}));

    // And an exact multiple below the origin does not lose a step.
    EXPECT_EQ(
