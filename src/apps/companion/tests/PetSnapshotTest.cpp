...
namespace
{
    ...
    // A collapse shortens the gauge itself rather than its contents.
    // Which is the one thing the picture has to be told about.
    TEST(PetSnapshotTest, ACollapsedCompanionCarriesAShorterCeiling)
    {
        Pet pet(kQuick);
        const Lineage lineage;
        pet.play();
    ...
    TEST(PetSnapshotTest, TheRecordBehindItRidesAlong)
    {
    ...
    TEST(PetSnapshotTest, TheSameCompanionSnapshotsToTheSameValue)
    {
        ...
        EXPECT_EQ(snapshotOf(pet, lineage), snapshotOf(pet, lineage));
    }
