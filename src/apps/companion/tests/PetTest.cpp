...
namespace
{
    // Small multiples of each other rather than the shipped numbers.
    // So a trace through a day can be read in one sitting.
    // The stages are pushed out of reach: growing up is its own test.
    ...
    // Nothing decays and nothing drains here.
    // So only what a test does to the companion can change it.
    ...
    // Every band set to one tick.
    // So a companion cheered up overnight cannot stop draining.
    ...
    // The second day of a companion's life is a heavy one.
    // Which is a fact about moodOn() rather than about this test.
    ...
    TEST(PetTest, Construction_StartsAwakeRestedAndAmused)
    {
    ...
    // A companion put to bed at full energy risks nothing at all.
    // Which is exactly the game this balance refuses.
    TEST(PetTest, Construction_RefusesABalanceWhereSleepIsFree)
    {
    ...
    TEST(PetTest, Construction_RefusesStartingAboveAMaximum)
    {
    ...
    // The base drain is one unit every so often.
    // How often is the band its happiness falls in.
    // Four bands, and four periods.
    TEST(PetTest, Step_HowFastEnergyGoesIsWhichBandItsHappinessIsIn)
    {
        ...
        // Happy: one unit every four ticks, so three of twelve.
        EXPECT_EQ(energyAfter(6, 12), 10U - 3U);

        // Content: one every three.
        EXPECT_EQ(energyAfter(4, 12), 10U - 4U);

        // Unhappy: one every two.
        EXPECT_EQ(energyAfter(2, 12), 10U - 6U);

        // Miserable is the fourth band, and no companion starts there:
        // happinessStart may not be zero, so it has to be worn down.
        PetConfig config = still();
        config.drainMiserableTicks = 1;
    ...
    TEST(PetTest, Step_BeingFamishedCostsHappinessAndEnergyTogether)
    {
        ...
        // Famished on the fourth tick, and the fifth is the first beat.
        stepTimes(pet, 4);
    ...
    // So it can take the last of one collapses have eaten into.
    // Nothing else has to happen at all.
    ...
    TEST(PetTest, Sleep_RecoversEnergyAndWakesWhenItIsFull)
    {
        ...
        // Spent by hand rather than by waiting: nothing drains here.
    ...
    // The knife edge of the whole game.
    // A game costing exactly what is left is allowed, and it drops it.
    // That is a risk somebody took.
    TEST(PetTest, Play_SpendingTheVeryLastOfItIsACollapse)
    {
        ...
        // A collapse is not an end, so what it said still stands.
        // Only perishing takes the bubble away.
        EXPECT_EQ(pet.saying(), Saying::Wheee);
    }
    ...
    // Every verb answers the same way while it is asleep.
    TEST(PetTest, AnyPressWhileAsleepWakesItUpInstead)
    {
        ...
        // And none of them counts as the thing it was asking for.
    ...
    TEST(PetTest, AnyPressAfterPerishingDoesNothing)
    {
        const PetConfig config = draining();
    ...
    TEST(PetTest, Saying_MurmursInItsSleepInstead)
    {
        ...
        // The game it was just played sits in the bubble for two ticks.
        // A line already up runs its course before another starts.
        stepTimes(pet, 4);
    ...
    TEST(PetTest, Saying_YawnsTheTickItBecomesTiredEnoughForBed)
    {
        ...
        // Ten energy, tired at four: the sixth tick is the crossing.
        stepTimes(pet, 5);
    ...
    // Said into an empty bubble only, so it never cuts a line off.
    ...
    TEST(PetTest, Stage_IsAFunctionOfTheTicksAndTheCeilingFollowsIt)
    {
        ...
        // An elder holds a child's ceiling rather than an adult's.
        ...
        // And a ceiling never goes below nothing at all.
    ...
    TEST(PetTest, Care_IsTheCountsItKeeps)
    {
        ...
        // One game against one slight is not twice as much good as bad.
        // Which is what Plain asks for.
        EXPECT_EQ(pet.form(), PetForm::Scruffy);
    }

    // A mood hurries one period along by a third of it.
    // A period of a single tick has no third, so it stays a tick.
    // A period of nothing would divide wrongly rather than play hard.
    ...
    // A companion may drop on the very tick it crossed the tired mark.
    // One that has has nothing left to yawn about.
    ...
    // Day zero is ordinary, and that is what keeps the rest readable.
    // Every trace above runs on the plain periods.
    // Nothing is hurried until a night has gone by.
    ...
    TEST(PetTest, Construction_RefusesAMemoryBeyondWhatItCouldHold)
    {
        PetMemory hungry;
    ...
    // Perished is exactly "the ceiling ran out", both ways round.
    ...
    // A companion left entirely alone has to run out of ceiling.
    // One played with, fed and put to bed on time has to be fine.
    // One played with without pause has to spend itself to death.
