...
namespace
{
    ...
    TEST(PetSinkTest, Handle_StepsTheCompanionOnlyOnATick)
    {
        Pet pet(kQuick);
        PetSink sink(pet);
    ...
    TEST(PropSinkTest, Handle_FeedsAHungryCompanionOnTheBowl)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
    ...
    TEST(PropSinkTest, Handle_PlaysOnAPressOnTheBall)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
        PropSink sink(pet, codec, kCanvas);
    ...
    TEST(PropSinkTest, Handle_SendsATiredCompanionToTheNest)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
        PropSink sink(pet, codec, kCanvas);
    ...
    TEST(PropSinkTest, Handle_TakesAPressOnNothingAsAProd)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
        PropSink sink(pet, codec, kCanvas);
    ...
    TEST(PropSinkTest, Handle_AnswersOnlyALeftPress)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
        PropSink sink(pet, codec, kCanvas);
    ...
    TEST(PropSinkTest, Handle_IgnoresAnEventThatIsNotInput)
    {
        Pet pet(kQuick);
        const InputEventCodec codec;
        PropSink sink(pet, codec, kCanvas);
    ...
    TEST(RenderSinkTest, Handle_DrawsAFrameOnATick)
    {
    ...
    TEST(RenderSinkTest, Handle_DrawsNothingWhenClosedOrUnticked)
    {
        const Pet pet(kQuick);
        const Lineage lineage;
        const antwika::companion::Translator translator{
            antwika::i18n::kDefaultLocale};
        const PetScene scene{translator};
        NiceMock<MockWindow> closed;
        ON_CALL(closed, isOpen()).WillByDefault(Return(false));
        EXPECT_CALL(closed, renderer()).Times(0);

        NiceMock<MockWindow> open;
        ON_CALL(open, isOpen()).WillByDefault(Return(true));
        EXPECT_CALL(open, renderer()).Times(0);

        const antwika::console::ConsolePicture consolePicture;
        RenderSink whileClosed(
            closed, scene, pet, lineage, kCanvas, consolePicture);
        whileClosed.handle(tick());

        RenderSink whileOpen(
            open, scene, pet, lineage, kCanvas, consolePicture);
        whileOpen.handle(other());
    }

    TEST(PacingSinkTest, Handle_WaitsOutTheIntervalOnlyOnATick)
    {
