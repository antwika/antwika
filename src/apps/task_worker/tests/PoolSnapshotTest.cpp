...
// A worker may hold a job the registry never saw submitted.
// It has no duration on record, so there is no progress to report.
...
// This is the ordering antwika::scheduler promises, drawn not asserted.
// Higher priority first, equal priority in submission order.
// Submission order is ascending JobId.
...
// Nothing the application can do reaches this.
// A submission naming an unknown dependency is refused before it lands.
// Kept blocked anyway, since an unrunnable dependency is not a reason.
...
TEST(PoolSnapshotTest, ViewEqualityComparesEveryField)
{
