...
namespace
{
    ...
    // Every line of a hand history follows from what the players did.
    // So the players are the first thing a test has to pin down.
    class ScriptedAgent final : public IAgent
    {
    public:
        void script(std::vector<Action> actions)
        {
    ...
    // Three seats, the button on bob, carol posting the small blind.
    ...
    // Bob opens, carol folds to his flop bet, alice calls him down.
    // The hand visits every street on the way.
...
// One whole hand against the exact text expected of it.
// Everything below only picks at a line this one does not reach.
...
// The short stack calls for everything it has.
// The rest of the raise it could not cover goes straight back.
...
// The deal was the whole hand, and it is still written up as one.
