...
namespace
{
    ...
    class FakeSpectatorBackend final : public IGfxBackend
    {
    public:
        explicit FakeSpectatorBackend(std::size_t closeAfterFrames)
        ...
        {
            auto owned = std::make_unique<FakeSpectatorWindow>(frames);
            window = owned.get();
            return owned;
        }
    ...
    private:
        class FakeSpectatorWindow final : public IWindow
        {
        public:
            explicit FakeSpectatorWindow(std::size_t &frames)
                : drawnInto(frames)
            {
            }
        ...
        private:
            ...
            {
            public:
                explicit FakeCountingRenderer(std::size_t &frames)
                    : frames(frames)
                {
                }
        ...
        std::size_t closeAt;
        std::size_t frames = 0;
        FakeSpectatorWindow *window = nullptr;
    };
...
TEST(ReplayIntegrationTest, Replay_ReproducesTheSameSession)
{
...
TEST(ReplayIntegrationTest, Recording_HoldsOnlyTheRoomsOwnInput)
{
    const auto script = liveScript();
...
TEST(ReplayIntegrationTest, Replay_ReproducesASessionEndedByAClose)
{
    FakeSpectatorBackend backend(30);
    FakeSleeper sleeper;
