...
namespace
{
    ...
    // Three hole-card pairs, then a five-card board.
    ...
    // Table and CashGame are both immovable, so neither is Room.
...
TEST(TableSnapshotTest, SnapshotOf_NamesWhoeverIsSeated)
{
    ...
    // A seat nobody bought into is reported, just with no name.
...
TEST(TableSnapshotTest, SnapshotOf_ReportsALiveHand)
{
    ...
    // Exactly one seat holds the button, and exactly one is asked to act.
...
TEST(TableSnapshotTest, SnapshotOf_ReportsTheBoardAsItComesOut)
{
    ...
    // Everyone just calls and checks, so the board runs out in full.
