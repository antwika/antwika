...
TEST(SudokuSolverIntegrationTest, EasyPuzzleMatchesKnownSolution)
{
    const Board solved = solve(kEasyPuzzle);
    EXPECT_EQ(solved.format(), kEasySolution);
}

TEST(SudokuSolverIntegrationTest, HarderPuzzleProducesAValidSolution)
{
    const Board solved = solve(kHardPuzzle);
    EXPECT_TRUE(isValidSolution(solved));

    // Also respects the puzzle's own givens.
