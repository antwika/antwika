...
namespace antwika::sudoku
{
     ...
     * to stop.
     *
     * **A puzzle has no end of its own** -- finishing one is a thing to
     * look at rather than a reason to close the window -- and under the
     * default `null` backend nothing else has one either: it reports no
     * window closing and no Escape, so a session there would run until
     * somebody interrupted it, and a `--record` run that is interrupted
     * never gets as far as writing its file.
     *
     * A stop appended here is ordinary external input: it goes through
     * the source the loop already pulls from, so it is dispatched like
     * any other event and lands in a recording, which is what makes a
     * capped session replayable.
     ...
     *
     * Line for line atlas_editor::TickLimitSource, which is the same
     * decision taken for the same reason one application over.
     */
