...
TEST(HoverTest, SaidOver_SaysAHousesPopulation)
{
    const Cell where{.x = 2, .y = 2};
...
TEST(HoverTest, SaidOver_SaysNoPopulationForAWell)
{
    const Cell where{.x = 2, .y = 2};
    const auto said = saidOver(
        where,
...
TEST(HoverTest, OperatorEquals_ComparesEveryReadoutField)
{
    const Cell where{.x = 2, .y = 2};
...
TEST(HoverTest, HoverFor_ChangesNothingAReplaySees)
{
