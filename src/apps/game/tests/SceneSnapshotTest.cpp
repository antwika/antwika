...
TEST(SceneSnapshotTest, SnapshotOf_TakesTheGoodAWalkerIsOutWith)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    const PathIndex paths;
    ...
    world.add<Walker>(
        entity,
        Walker{.kind = antwika::game::WalkerKind::CartPusher, .carried = 70});
    world.add<antwika::game::Errand>(
        entity,
        antwika::game::Errand{
            .carrying = antwika::game::Resource::Clay,
            .leg = antwika::game::ErrandLeg::Outbound});
    world.commit();

    const auto snapshot = snapshotOf(world, paths, Camera(), kExtent);

    ASSERT_EQ(snapshot.walkers.size(), 1U);
    EXPECT_EQ(snapshot.walkers[0].carrying, antwika::game::Resource::Clay);
    EXPECT_EQ(snapshot.walkers[0].carried, 70);
}

TEST(SceneSnapshotTest, SnapshotOf_LeavesAWalkerOnNoErrandCarryingNothing)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    const PathIndex paths;
    ...
    ASSERT_EQ(snapshot.walkers.size(), 1U);
    EXPECT_FALSE(snapshot.walkers[0].carrying.has_value());
}
...
TEST(SceneSnapshotTest, OperatorEquals_ComparesEveryWalkerSpriteField)
{
    ...
    const auto twin = sprite;
    EXPECT_EQ(sprite, twin);
    ...
    auto laden = sprite;
    laden.carrying = antwika::game::Resource::Clay;
    EXPECT_NE(sprite, laden);
}
...
TEST(SceneSnapshotTest, OperatorEquals_ComparesBothWalkerViewFields)
{
...
TEST(SceneSnapshotTest, OperatorEquals_ComparesEverySnapshotField)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    PathIndex paths;
...
TEST(SceneSnapshotTest, OperatorEquals_ComparesTheExtentAndWalkers)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    const PathIndex paths;
...
TEST(SceneSnapshotTest, OperatorEquals_ComparesEverySummaryField)
{
    using antwika::game::Camera;
...
TEST(SceneSnapshotTest, SnapshotOf_TakesAHousesLevelOntoBoth)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    const PathIndex paths;
...
TEST(SceneSnapshotTest, SnapshotOf_TakesAHousesPeopleOntoBoth)
{
    NiceMock<MockLogger> logger;
    World world(logger);
    const PathIndex paths;
