...
namespace
{
    // A camera at every zoom level, so nothing here tests one scale.
...
// The middle level is where art pixels are screen pixels.
// Either side of it halves or doubles, never rounds.
TEST(SpriteBoundsTest, ScaledToZoom_IsExactAtEveryLevel)
{
    ...
    // The whole tile stays the whole tile at every level.
...
// A cell's anchor is its own diamond's bottom corner.
// Said against cellBounds(), which is what culling and clicks use.
...
// A block's anchor is the bottom corner of the whole block's diamond.
// Said against footprintBounds(), the box the block's outline traces.
...
// The pivot lands on the anchor, scaled with everything else.
TEST(SpriteBoundsTest, SpriteBounds_AnchorsThePivotAtEveryZoom)
{
    ...
    for (std::size_t level = 0; level < kZoomHalfWidths.size(); ++level)
    {
              ...
              AtlasKind::ThreeByThree})
        {
            ...
            const auto box = spriteBounds(kind, anchor, camera);
...
// One pinned picture, so the arithmetic cannot silently flip.
// The default zoom doubles the art, so a 64 by 96 sprite fills 128 by 192.
// Its pivot lands on the origin cell's bottom corner at (0, 32).
...
// A 1x1 building's box is exactly the 1x1 tile's box.
// The house and the ground share a sheet, so they share a geometry.
TEST(SpriteBoundsTest, BuildingSpriteBounds_MatchesTheTileForOneByOne)
{
    ...
    EXPECT_EQ(
        buildingSpriteBounds(at, BuildingKind::House, camera),
        tileSpriteBounds(at, camera));
}

// A block's sprite is anchored to the block's own bottom corner.
// Its diamond then covers exactly the footprint's box.
TEST(SpriteBoundsTest, BuildingSpriteBounds_CoversTheFootprintExactly)
{
         ...
         {BuildingKind::House, BuildingKind::Farm, BuildingKind::Storage})
    {
        ...
        const auto sprite = buildingSpriteBounds(origin, kind, camera);

        // The sprite's margins reach one half-tile past either side.
        EXPECT_EQ(
            sprite.origin.x,
            block.origin.x
                - static_cast<std::int32_t>(camera.halfWidth()));
        ...
        // The skirt hangs two half-heights below the block's box.
        EXPECT_EQ(
            sprite.origin.y
