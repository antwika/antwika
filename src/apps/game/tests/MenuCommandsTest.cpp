...
// A new game is an empty grid, through the very store a load uses.
...
// Put back where a run starts it rather than to the origin.
// The origin is off the grid entirely -- see kInitialPan in main().
...
// A new game does not change which screen is up.
// It is asked for from the city, and it hands a city back.
...
// Put away before the map is asked for, exactly as the key does.
// A city left open would be stepped and drawn while another is showing.
...
// A change is staged rather than applied.
// So the click asking for a screen is not also read by it.
// See AppMode.hpp.
