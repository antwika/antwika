...
// Two stores the same distance away, and the answer is the lower cell.
// Built in the order that would give the other one if the view decided.
...
// A market is supplied by its own buyer rather than by a cart.
...
// The nearer store is the one built second, and the higher cell.
// So the answer is not "whichever the map handed over first".
