...
TEST(StoreTableTest, FetchesFromStores_NamesTheClayOfAWorkshop)
{
    EXPECT_TRUE(fetchesFromStores(BuildingKind::Workshop, Resource::Clay));
    EXPECT_FALSE(fetchesFromStores(BuildingKind::Workshop, Resource::Food));
    EXPECT_FALSE(
        fetchesFromStores(BuildingKind::Workshop, Resource::Pottery));
}

TEST(StoreTableTest, FetchesFromStores_SendsAMarketForFoodAndPottery)
{
    EXPECT_TRUE(fetchesFromStores(BuildingKind::Market, Resource::Food));
    EXPECT_TRUE(fetchesFromStores(BuildingKind::Market, Resource::Pottery));
    EXPECT_FALSE(fetchesFromStores(BuildingKind::Market, Resource::Clay));
}

TEST(StoreTableTest, FetchesFromStores_NamesEveryKindThatShops)
{
    for (std::size_t index = 0; index < kBuildingKindCount; ++index)
    {
        const auto kind = static_cast<BuildingKind>(index);
        const auto shops = kind == BuildingKind::Workshop
            || kind == BuildingKind::Market;

        EXPECT_EQ(fetchesFromStores(kind), shops) << index;

        bool any = false;

        for (const auto resource : kResources)
        {
            any = any || fetchesFromStores(kind, resource);
        }

        EXPECT_EQ(any, shops) << index;
    }
}
...
TEST_F(StoreTest, StockOf_GivesEachResourceASlotOfItsOwn)
{
    const auto entity = world.create();
    world.add<Cell>(entity, Cell{});
    world.add<Building>(
    ...
    EXPECT_EQ(kResources.size(), 3U);
    EXPECT_EQ(stockOf(world, entity, Resource::Food), 1);
    EXPECT_EQ(stockOf(world, entity, Resource::Clay), 2);
    EXPECT_EQ(stockOf(world, entity, Resource::Pottery), 3);
}
