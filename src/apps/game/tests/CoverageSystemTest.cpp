...
TEST_F(CoverageSystemTest, Update_ReachesTheSameStateUnderEitherOrder)
{
    ...
    walk(world, Cell{.x = 2, .y = 0}, WalkerKind::WaterCarrier);
    world.commit();

    walk(other, Cell{.x = 2, .y = 0}, WalkerKind::WaterCarrier);
    ...
    EXPECT_EQ(coverageOf(world, first, Service::Water), kCoverageFull);
    EXPECT_EQ(coverageOf(world, second, Service::Water), kCoverageFull);
