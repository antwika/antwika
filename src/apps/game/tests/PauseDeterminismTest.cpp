        ...
        bool pausing, antwika::time::Tick stopAt = 20)
    {
        ...
        events.push_back(
            TickEvent{
                .tick = 0,
                .event = pressAt(
                    Cell{.x = 2, .y = kRoadRow}, MouseButton::Left)});
...
TEST(PauseDeterminismTest, Run_WalksSomewhereWhenUnpaused)
{
...
TEST(PauseDeterminismTest, Run_HoldsTheWalkersWhenPaused)
{
...
TEST(PauseDeterminismTest, Run_MovesAWalkerOverTwentyTicks)
{
...
TEST(PauseDeterminismTest, Run_StillBuildsAndPansWhilePaused)
{
...
TEST(PauseDeterminismTest, Run_APausedRunReplaysToTheSameState)
{
    auto script = session(true);
