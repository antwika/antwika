...
{
    ReplaySource inner({TickEvent{.tick = 0, .event = kToggle}});
    ...
    EXPECT_EQ(source.eventsFor(0), (std::vector<Event>{kToggle}));
}
    ...
    EventsFor_AppendsAnEngineStopAfterTheInnerEventsOnACloseRequest)
{
    ...
    EXPECT_EQ(source.eventsFor(0), (std::vector<Event>{kToggle, kStop}));
}
