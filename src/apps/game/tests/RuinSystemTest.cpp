...
TEST_F(RuinSystemTest, Update_TurnsABurntOutFireToDebris)
{
    ...
    // One tick spends the last of the countdown.
    // The next one finds it spent and puts the fire out.
    run(2);
...
TEST_F(RuinSystemTest, Update_BreaksADistanceTieByAscendingCell)
{
    ...
    // Both two strides off; (1,1) orders below (3,3).
...
TEST_F(RuinSystemTest, Update_BreaksASharedCellTieByAscendingEntity)
{
    ...
    // Two firemen on one cell: the lower handle answers.
    const auto first =
...
{
    // The fire burns on with no fireman anywhere.
    // The one dispatched already stays on his own call.
    ...
    // Still his first fire, and the second one waits for help.
...
TEST_F(RuinSystemTest, Update_SendsOneFiremanPerFireAndNoMore)
{
    ...
    // Both stand nearest to the western fire.
    // The fires are answered in ascending Cell order.
    // So the western one chooses first.
    const auto closer =
...
TEST_F(RuinSystemTest, KeepsNoRuinOnAnEntityAlreadyRetired)
{
    const auto entity = world.create();
...
TEST_F(RuinSystemTest, KeepsNoFireCallOnAWalkerAlreadyRetired)
{
    const auto fireman =
        walkerOf(Cell{.x = 1, .y = 1}, WalkerKind::Fireman);
...
// The candidates arrive in cell order, which is not distance order.
// A nearer man later in the walk must still win the call.
TEST_F(RuinSystemTest, Update_FindsANearerFiremanLaterInCellOrder)
{
    const auto fire = smoulder(
        Cell{.x = 5, .y = 6}, Ruin{.kind = BuildingKind::House});

    // (1,9) orders below (5,5) as a Cell, and stands further off.
