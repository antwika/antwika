...
TEST_F(HotkeySinkTest, Handle_PausesAndResumesOnThePauseKey)
{
...
TEST_F(HotkeySinkTest, Handle_TheZoomKeysMoveTheCamera)
{
...
TEST_F(HotkeySinkTest, Handle_ResetsTheCameraToTheOpening)
{
    camera.panBy(64, 64);
    camera.zoomIn();
...
TEST_F(HotkeySinkTest, Handle_FollowsARebindingFromThenOn)
{
    EXPECT_EQ(
...
TEST_F(HotkeySinkTest, Handle_IgnoresAnUnboundKey)
{
    press(Key::J);
...
TEST_F(HotkeySinkTest, Handle_AnswersAHeldKeyOnce)
{
    press(kDefaultBindings.keyFor(Action::Pause));
    ASSERT_TRUE(pause.paused());
...
TEST_F(HotkeySinkTest, Handle_IgnoresInputThatIsNotAKeyPress)
{
    send(
...
TEST_F(HotkeySinkTest, Handle_AnEventThatIsNotInputIsIgnored)
{
    tick();
