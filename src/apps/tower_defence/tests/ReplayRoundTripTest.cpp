...
namespace
{
    ...
    // Two small levels, so the run reaches the second inside the cap.
    // And so the solver stays cheap under the coverage build's -O0.
    ...
    // Cells beside the road, which is where a tower is worth having.
    // Worked out from a campaign built with the same config.
    // So the script survives a change of seed.
    ...
    // One number per tick per thing worth reproducing, in tick order.
    // A summary can agree over two runs that diverged and met again.
    // This cannot.
    using Trace = std::vector<std::uint64_t>;

    class Tracer final : public antwika::event::ITickEventSink
    {
    public:
        Tracer(const Campaign &campaign, Trace &trace)
        ...
        TickEventRecorder *recorder)
    {
        ...
        TowerDefenceWiring config{
            ...
            { return std::make_unique<Tracer>(campaign, trace); }};
    ...
    // What a recorded session replayed out of a file has to come to.
    // Which is the reason this project exists.
    //
    // Nothing about the campaign is in the file.
    // Not the level, not the wave a tick released.
    // Not the kind of anything in it, not the damage, not the score.
    // All of it is regenerated from the seed and the clicks.
    // A clock or a file creeping in would part the two runs here.
    TEST(ReplayRoundTripTest, ARecordedSessionReplaysToTheSameCampaign)
    {
        TickEventRecorder recorder;
        Trace liveTrace;
        ...
        // A recording keeps the run's input.
        // So the engine's own ticks are left out on the way to a file.
        ...
        // A scoreless run would agree with itself over nothing.
