...
using antwika::tower_defence::configFromJson;
using antwika::tower_defence::configToJson;
using antwika::tower_defence::kConfigFormatVersion;
using antwika::tower_defence::kConfigMagic;
using antwika::tower_defence::loadConfigFileOrDefaults;
using antwika::tower_defence::readConfig;
using antwika::tower_defence::standardConfigMigrations;
using antwika::tower_defence::writeConfig;
...
namespace
{
    ...
    [[nodiscard]] TowerDefenceConfig retuned()
    {
    ...
    void expectEqual(
        const TowerDefenceConfig &decoded,
        const TowerDefenceConfig &expected)
    {
        EXPECT_EQ(decoded.startingLives, expected.startingLives);
        EXPECT_EQ(decoded.framePeriodMs, expected.framePeriodMs);
    }

    void expectDefaults(const TowerDefenceConfig &decoded)
    {
        EXPECT_EQ(decoded.startingLives, TowerDefenceConfig{}.startingLives);
        EXPECT_EQ(decoded.framePeriodMs, TowerDefenceConfig{}.framePeriodMs);
    }
    ...
    protected:
        ...
        std::filesystem::path directory{
            ...
            / ("antwika-tower_defence-config." + std::to_string(::getpid()))};
    };
} // namespace

TEST_F(ConfigFileTest, ADocumentStatesItsFormatAndItsVersion)
{
    const auto encoded = configToJson(TowerDefenceConfig{});
...
{
    auto document = configToJson(TowerDefenceConfig{});
