...
namespace
{
    ...
    // A window that really remembers, since the toggle reads it back.
    ...
    public:
        explicit FullscreenState(MockWindow &window)
        {
            ...
            ON_CALL(window, setFullscreen(::testing::_))
                .WillByDefault([this](bool wanted) { filling = wanted; });
        }
...
{
    ReplaySource inner({at(0, kCodec.encode(KeyPressed{.key = Key::F10}))});
    NiceMock<MockWindow> window;
    const FullscreenState state(window);

    FullscreenToggleSource source(inner, window, kCodec, Key::F10);

    const auto events = source.eventsFor(0);

    EXPECT_TRUE(state.filled());

    // A pure observer: the stream is what the inner source returned.
    EXPECT_EQ(
        events,
...
// Holding the key is one toggle rather than a flicker.
...
// So a bad payload throws out of here rather than reaching a window.
