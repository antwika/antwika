...
namespace
{
    ...
    // The pointer moves across two buttons and presses the far one.
    // One round per tick, so the motion really is spread over ticks.
    ...
    // Two buttons the UI itself knows nothing about the pointer for.
    // The recorded stream is gated, so the frame is drawn with none.
    ...
    // What a run puts into TickEventRecorder, tick by tick.
    // Which is exactly what a --record file would then hold.
    struct Run
    {
    ...
    // One run of the script, with or without a hover pass on it.
    // Nothing else differs, so any difference is the hover's doing.
...
{
    // The claim: hover is not part of the simulation.
    // So a session recorded with it holds what one without it holds.
    // Nothing is added, nothing is thinned, nothing moves tick.
...
{
    // The other half of the claim, so the first cannot pass vacuously.
    // Six movements went in and one came out, on the press's own tick.
    // IdleMotionSource latched the last and released it at the press.
    // Everything before that is in no recording at all.
    // The pictures below follow all of it all the same.
    const auto hovering = drive(true);
...
TEST(HoverRecordingTest, Picture_FollowsThePointerTheRecordingNeverSaw)
{
    ...
    // Nothing reaches the UI's own pointer here.
    // So without the pass every frame is the same picture.
    EXPECT_EQ(1U, distinctPictures(plain));

    // With it, the highlight walks from one button to the other.
