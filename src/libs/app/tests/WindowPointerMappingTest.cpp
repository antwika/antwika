...
TEST(WindowPointerMappingTest, ToSurface_ChangesNothingUnderTheNullCase)
{
    ...
    const WindowPointerMapping mapping(window, kCanvas);

    EXPECT_EQ(
        mapping.toSurface(Position{.x = 300, .y = 200}),
...
TEST(WindowPointerMappingTest, ToSurface_UndoesTheScaleTheViewportApplies)
{
    ...
    EXPECT_EQ(
        mapping.toSurface(Position{.x = 600, .y = 400}),
        (Position{.x = 300, .y = 200}));
}

// The pillarboxed case: the picture is centred, so the bar is subtracted.
TEST(WindowPointerMappingTest, ToSurface_SubtractsTheOffsetOfTheBars)
{
    ...
    // 448 pixels of bar down each side, and no scaling at all.
    EXPECT_EQ(
        mapping.toSurface(Position{.x = 448, .y = 0}),
        (Position{.x = 0, .y = 0}));
    EXPECT_EQ(
        mapping.toSurface(Position{.x = 100, .y = 10}),
        (Position{.x = -348, .y = 10}));
}

// Read afresh every call rather than captured at construction.
// So dragging an edge mid-session is handled by the next click.
TEST(WindowPointerMappingTest, ToSurface_FollowsAWindowThatChangedSize)
{
    ...
    EXPECT_EQ(
        mapping.toSurface(Position{.x = 512, .y = 320}),
