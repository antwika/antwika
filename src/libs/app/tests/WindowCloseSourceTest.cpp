...
namespace
{
    ...
    [[nodiscard]] WindowEvent closeOf(WindowId idWindow)
    {
        return WindowEvent{.window = idWindow, .payload = CloseRequested{}};
    }
}
...
TEST_F(WindowCloseSourceTest, EventsFor_PassesTheWrappedEventsThrough)
{
    ...
    WindowCloseSource source(innerSource, backend, window);
