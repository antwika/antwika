...
namespace
{
    ...
    // Everything a window a test opens has to answer.
    // Its reported size is the canvas unless a case says otherwise.
    // So a mapped position is the identity until one asks for a scale.
    class Fixture
    {
    public:
        Fixture()
    ...
    // A recording of one event, written where only this case will look.
...
TEST(WindowedSessionTest, AnnouncesBothBackendsUnderTheApplicationsName)
{
    Fixture fixture;
...
TEST(WindowedSessionTest, AsksForTheTitleSizeAndResizabilityItWasGiven)
{
    Fixture fixture;
    FakeInputBackend input;
...
// What an application lays out afterwards is laid against this.
// Its own debug-console picture is the one every app on this builds.
TEST(WindowedSessionTest, HandsBackTheCanvasItAskedTheWindowFor)
{
    Fixture fixture;
    FakeInputBackend input;
...
TEST(WindowedSessionTest, HandsBackTheCodecItsPipelineEncodesWith)
{
    Fixture fixture;
    FakeInputBackend input;
    ...
    EXPECT_EQ(session.codec().decode(session.codec().encode(edge)), edge);
}

TEST(WindowedSessionTest, ReadsTheDeviceWhenNoReplayWasNamed)
{
    Fixture fixture;
    const InputEvent edge{KeyPressed{.key = Key::A}};
...
TEST(WindowedSessionTest, ReadsNoDeviceWhenAReplayWasNamed)
{
    Fixture fixture;
...
TEST(WindowedSessionTest, SeedsFromTheDemoRecordingWhenNoneWasNamed)
{
    Fixture fixture;
...
// A named recording that will not read is a failed run.
// The window and the picture already built go back out with it.
TEST(WindowedSessionTest, ThrowsWhenTheNamedRecordingCannotBeRead)
{
    Fixture fixture;
    FakeInputBackend input;
...
TEST(WindowedSessionTest, ReadsADevicePositionAsOneOnTheCanvasWhenAsked)
{
    Fixture fixture;
...
TEST(WindowedSessionTest, LeavesADevicePositionAloneWhenItIsNotAsked)
{
    Fixture fixture;
    FakeInputBackend input({InputEvent{
        PointerMoved{.position = Position{.x = 200, .y = 100}}}});
...
// Every other field of the options is passed through untouched.
// Coalescing is the one whose absence apps/life depends on.
TEST(WindowedSessionTest, PassesTheApplicationsOwnInputPolicyThrough)
{
    Fixture fixture;
...
// Closing the window ends the run, in every application on this.
TEST(WindowedSessionTest, EndsTheRunWhenItsOwnWindowIsAskedToClose)
{
    Fixture fixture;
    FakeInputBackend input;
...
TEST(WindowedSessionTest, ReportsTheBackendThatDrawsNothingAsDrawingNothing)
{
    Fixture fixture;
    FakeInputBackend input;
...
TEST(WindowedSessionTest, ReportsAnyOtherBackendAsDrawingSomething)
{
    Fixture fixture;
    FakeInputBackend input;
