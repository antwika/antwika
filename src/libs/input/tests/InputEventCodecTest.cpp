...
namespace
{
    ...
    // The failing tests below want the throw, not the decoded value.
    // Going through a function is what stops the compiler objecting.
...
// This is the property the whole replay bridge rests on.
// Whatever a backend reported has to survive being written down.
...
// The payload is part of the replay format.
// So its exact shape is worth pinning down, not only round-tripping.
...
// A coordinate too big for a Position would be narrowed on the way out.
...
// Both key events reject an unknown name, not just the one that repeats.
...
// Both button events reject an unknown name, not just the release.
...
// The message says which event failed.
// So a bad replay names the line to look at, not only its own shape.
