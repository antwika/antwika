...
namespace
{
    ...
    // Halves both axes, so a mapped position is obviously mapped.
    // Integer throughout, as every implementation must be.
    class Halving final : public IPointerMapping
    {
    public:
...
// A key and a wheel notch happen wherever the pointer already was.
// Neither carries a position, so neither is rewritten.
...
// Anything that is not this library's passes through untouched.
...
// No event is added, dropped, reordered or moved to another tick.
...
// So a bad payload throws out of here rather than being quietly passed on.
