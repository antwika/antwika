...
{
    const auto pastEndButton = static_cast<MouseButton>(kMouseButtonCount);

    EXPECT_THROW(static_cast<void>(toString(pastEndButton)), InputError);
}
...
TEST(MouseButtonTest, ToString_SaysWhichValueItRefusedToName)
{
    ...
    try
    {
        static_cast<void>(toString(pastEndButton));
