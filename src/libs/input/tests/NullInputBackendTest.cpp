...
TEST(NullInputBackendTest, Construction_LogsThatItReportsNothing)
{
    MockLogger logger;
...
TEST(NullInputBackendTest, Capabilities_ClaimsBothDevices)
{
    ...
    // Both rather than neither, so a headless replay binds what live bound.
    EXPECT_EQ(
...
TEST(NullInputBackendTest, PollEvent_AlwaysReturnsNullopt)
{
    ...
    // Drains rather than blocks, so a caller emptying it terminates.
