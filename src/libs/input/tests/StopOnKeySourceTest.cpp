...
{
    ReplaySource inner({keyDown(Key::Escape)});
    const InputEventCodec codec;
    StopOnKeySource source(inner, codec, Key::Escape);
...
{
    ReplaySource inner({keyDown(Key::W)});
    const InputEventCodec codec;
    StopOnKeySource source(inner, codec, Key::Escape);

    EXPECT_FALSE(holdsStop(source.eventsFor(0)));
}

TEST(StopOnKeySourceTest, EventsFor_IgnoresARepeatOfTheChosenKey)
{
    ReplaySource inner({keyDown(Key::Escape, true)});
    const InputEventCodec codec;
    StopOnKeySource source(inner, codec, Key::Escape);
...
{
    ReplaySource inner({keyDown(Key::Escape), keyDown(Key::Escape)});
    const InputEventCodec codec;
    StopOnKeySource source(inner, codec, Key::Escape);
