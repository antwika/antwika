...
namespace
{
    ...
    constexpr auto kJustPastTheLastButton =
        static_cast<MouseButton>(antwika::input::kMouseButtonCount);
}
...
TEST(MouseTest, Apply_IgnoresTheFirstButtonPastTheEnumerationOnRelease)
{
    ...
    mouse.apply(PointerButtonReleased{.button = kJustPastTheLastButton});

    EXPECT_FALSE(mouse.wasReleased(kJustPastTheLastButton));
}
