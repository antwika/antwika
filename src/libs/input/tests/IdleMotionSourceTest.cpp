...
{
    // A scroll carries no position, so it reads the folded one.
    // Holding this movement back for good would anchor a zoom elsewhere.
    ReplaySource inner(
        {at(0, move(7, 8)),
...
{
    // Nothing the codec recognises, so nothing it can rule out either.
    ReplaySource inner(
...
{
    // Both paths run the same pipeline, so a replay meets this twice.
