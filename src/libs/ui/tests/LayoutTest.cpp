...
namespace
{
    ...
    // A glyph cell is kGlyphAdvance by kGlyphLineHeight.
    // So "ab" at scale one covers 12 by 8.
...
TEST(LayoutTest, Arrange_GivesEachGrowerAnEqualShareOfTheRoom)
{
    ...
    // 100 does not divide by three.
    // The pixels left over go to the earliest children, by rule.
...
// Proportional rather than first-come-first-served.
// A container with too little room still shows every child.
...
TEST(LayoutTest, Measure_AddsPaddingAndGapsAroundAColumnOfText)
{
    ...
    // Two lines of eight, one gap of three, two pixels of padding.
    EXPECT_EQ(
...
// A growing child reports its content as a minimum.
// So a container fitting around one does not collapse to nothing.
...
TEST(LayoutTest, Arrange_AlignsToTheEndAndClampsAnOversizedChild)
{
    ...
    // Clamped to the container rather than allowed to escape it.
    // Nothing here can clip what would spill out.
    EXPECT_EQ(
        (Rect{
            .origin = {.x = 10, .y = 0},
...
TEST(LayoutTest, Layout_PlacesAGrandchildInsideItsParentsContentBox)
{
    ...
    // Inside the row's padding, centred on its 30-pixel content height.
    EXPECT_EQ(
        (Rect{
            .origin = {.x = 5, .y = 16},
    ...
    // After the label and the gap, taking the rest of the content width.
    EXPECT_EQ(
        (Rect{
            .origin = {.x = 27, .y = 10},
    ...
    // The fixed row took 40, so the growing sibling gets the other 60.
    EXPECT_EQ(
        (Rect{
            .origin = {.x = 0, .y = 40},
