...
TEST(LayoutTreeTest, Construct_HoldsJustTheRoot)
{
...
TEST(LayoutTreeTest, Construct_OpensTheRoot)
{
    const LayoutTree tree{root()};
...
TEST(LayoutTreeTest, Open_MakesTheNewContainerTheOpenOne)
{
    ...
    const auto opened = tree.open(Node{.axis = Axis::Row});

    EXPECT_EQ(opened, tree.openIndex());
    EXPECT_EQ(0U, tree.node(opened).parent);
}
...
TEST(LayoutTreeTest, Open_TakesSubsequentChildren)
{
    ...
    EXPECT_EQ(opened, tree.node(leaf).parent);
    EXPECT_EQ(leaf, tree.node(opened).firstChild);
...
// Every child is appended after its parent.
// Both layout passes rely on that to be flat loops, not recursion.
