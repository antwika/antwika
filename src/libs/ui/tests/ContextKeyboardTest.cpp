...
namespace
{
    ...
    // That puts the first button at (0, 0) and makes it twelve by eight.
    ...
    // The four bars a ring of this thickness draws round this box.
    // In the order flatten() appends them.
...
// A caller reporting no keyboard draws what it drew before.
...
// Enter reports through the field a press already uses.
// So a caller handles a click and a keystroke in one place.
...
// A widget narrower than the ring is filled by it, not escaped by it.
// There is no clipping anywhere in this library.
...
// The pointer takes focus with it, so the ring follows the click.
