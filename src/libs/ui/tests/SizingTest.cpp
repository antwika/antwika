...
TEST(SizingTest, Equality_IsTrueForTheSameModeAndPixels)
{
    const auto left = fixedSize(40);
    const auto right = fixedSize(40);

    EXPECT_EQ(left, right);
}

TEST(SizingTest, Equality_IsFalseWhenTheModeDiffers)
{
...
TEST(SizingTest, Equality_IsFalseWhenThePixelsDiffer)
{
    const auto left = fixedSize(40);
