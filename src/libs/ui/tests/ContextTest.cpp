...
namespace
{
    ...
    // No padding and no gaps anywhere.
    // So every expected number below is the layout's own arithmetic.
...
TEST(ContextTest, Context_PutsTheThemeGapBetweenChildren)
{
    ...
    // Eight pixels of text, then the theme's five.
    EXPECT_EQ(
        (DrawText{
            .origin = {.x = 0, .y = 13},
            .text = "cd",
...
// Nothing is consumed by asking, so a caller may ask again.
