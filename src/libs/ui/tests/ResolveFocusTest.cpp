...
namespace
{
    ...
    // Three focusable widgets, in declaration order.
    // Plus a label and an unnamed button for the order to step over.
...
// The tab order is declaration order.
// An unnamed button and a label are stepped over, not stopped at.
...
// Two nodes sharing an id are one widget, so Tab stops at it once.
...
// The layout is described afresh every frame.
// A focus on a widget no longer declared is dropped, so Tab restarts.
...
// The keys are applied in the order they arrived.
// That is the whole reason they are a list rather than flags.
...
// A press takes focus along with it.
...
TEST(ResolveFocusTest, Resolve_MovesFocusToWhatThePointerActivated)
{
    ...
    // Every widget here is arranged over the same box.
    // So the topmost one is the last declared.
    const auto interactions = resolve(
        tree,
...
// Two nodes sharing an id are one widget, so both take the ring.
// The way both already take the resolved background.
...
// Focus is dressed alongside the pointer's appearance, not instead.
// So a focused button still lights up when it is hovered.
