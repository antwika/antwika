...
namespace
{
    ...
    /**
     * @brief Find the fill a colour was painted with.
     * @param frame The finished frame to search.
     * @param color The colour to look for.
     * @return The rectangle it filled, or nothing when it was not drawn.
     */
...
TEST(WidgetRectsTest, Equality_ComparesTheEntriesAndTheirOrder)
{
    const Rect box{
    ...
    // The same widget somewhere else is a different answer.
    EXPECT_FALSE(one == moved);
}

// The default is no mapping at all.
// Every other layout test calls it that way and pays nothing for it.
...
// The whole point of reading the layout back rather than repeating it.
// A container with too little room cuts its children in proportion.
// An application repeating that sum would place art at sizes nobody got.
TEST(WidgetRectsTest, Layout_ReportsTheShrunkRectangleAndNotTheAskedFor)
{
    ...
    // 120 asked for into 50 of room gives 17, 17 and 16.
    // The leftover pixel goes to the earliest child, by rule.
...
// Two nodes sharing an id are one widget, so one entry answers for both.
// The entry kept is the one declared last.
...
// The reported rectangle and the drawn one are one value.
// Both are read off the same arranged node.
...
// Not only containers: a button carries an id already, so it answers too.
// That is what an application drawing art behind one needs.
...
// Containment is the layout's job, since antwika::gfx has no clipping.
// The mapping inherits that rather than promising it separately.
...
// An application names a container to place its own art against it.
// An id means one thing here, so the pointer reports it as well.
