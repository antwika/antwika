...
TEST_F(ConfigDocumentTest, DocumentSchema_DeclaresDraftSevenAndTheFormat)
{
    const auto schema = documentSchema(kFormat, "test config document");

    EXPECT_EQ(
        schema.at("$schema").get<std::string>(),
        "http://json-schema.org/draft-07/schema#");
    EXPECT_EQ(
        schema.at("title").get<std::string>(), "test config document");
    EXPECT_EQ(schema.at("type").get<std::string>(), "object");
    EXPECT_EQ(
        schema.at("properties").at("magic").at("const")
            .get<std::string>(),
        kFormat.magic);
    EXPECT_EQ(
        schema.at("properties").at("version").at("const")
            .get<std::uint32_t>(),
        kFormat.version);
}
...
TEST_F(ConfigDocumentTest, WriteConfig_RoundTripsThroughAStream)
{
    auto document = newDocument(kFormat);
    document["periodTicks"] = 5;
...
TEST_F(ConfigDocumentTest, WriteConfig_IndentsByTwoAndEndsWithANewline)
{
    const auto document = newDocument(kFormat);
