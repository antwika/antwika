...
// capabilities() says this backend does not connect -- see NullHost.
...
// Every one of these is an ordinary no-op rather than a refusal.
// Holding no peers is a state this host is in, not a failure.
TEST(NullHostTest, SendingAndDisconnectingDoNothingAtAll)
{
    NullHost host{Endpoint{kHere}};
