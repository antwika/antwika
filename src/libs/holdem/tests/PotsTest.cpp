...
// Somebody bets more than anyone left could cover, then folds.
// Those chips have no layer of their own, so they ride on top.
...
TEST(PotsTest, DistributePots_GivesAnOddChipToTheFirstSeatLeftOfTheButton)
{
    ...
    // Seat 1 is nearest the left of the button, so it takes the odd chip.
...
TEST(PotsTest, DistributePots_AwardsEachLayerToItsOwnBestContender)
{
    ...
    // Seat 0 has the best hand overall, but is only in the first layer.
