...
TEST(DeckTest, Shuffle_DealsTheOrderRecordedForSeed2026)
{
    SplitMix64Rng rng(2026);
    Deck deck(rng);
    deck.shuffle();

    const auto dealt = dealAll(deck);
    const auto recorded = parseCards("Td Jh 6c Qs 7d");
    ASSERT_GE(dealt.size(), recorded.size());
    EXPECT_EQ(
        std::vector<Card>(dealt.begin(), dealt.begin() + recorded.size()),
        recorded);
}
