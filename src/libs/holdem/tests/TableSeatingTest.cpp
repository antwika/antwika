...
TEST(TableSeatingTest, Ctor_RejectsATableWithTooFewSeats)
{
    EXPECT_THROW(Table(1, kBlinds), TableStateError);
}

TEST(TableSeatingTest, Ctor_RejectsATableWithTooManySeats)
{
    EXPECT_THROW(Table(10, kBlinds), TableStateError);
}

TEST(TableSeatingTest, Ctor_RejectsASmallBlindAboveTheBigBlind)
{
    EXPECT_THROW(
        Table(2, Blinds{.small = 10, .big = 5}), TableStateError);
}

TEST(TableSeatingTest, Ctor_AcceptsBlindsOfTheSameSize)
{
...
TEST(TableSeatingTest, Ctor_LeavesEverySeatEmpty)
{
...
TEST(TableSeatingTest, FirstFreeSeat_FindsSeatZeroAtAnEmptyTable)
{
    const Table table(3, kBlinds);

    ASSERT_TRUE(table.firstFreeSeat().has_value());
    EXPECT_EQ(*table.firstFreeSeat(), makeSeatId(0));
}
...
TEST(TableSeatingTest, RemovePlayer_RefusesAFolderWhoseOneChipIsStillIn)
{
    Table table(3, Blinds{.small = 1, .big = 2});
    table.seatPlayer(makeSeatId(0), 100);
    table.seatPlayer(makeSeatId(1), 100);
    table.seatPlayer(makeSeatId(2), 100);
    auto deck = threePlayerDeck();
    ...
    ASSERT_TRUE(table.isHandInProgress());
    ASSERT_FALSE(table.seatAt(makeSeatId(2)).inHand);
    ASSERT_EQ(table.seatAt(makeSeatId(2)).committed, 1U);
    EXPECT_THROW(table.removePlayer(makeSeatId(2)), TableStateError);
}
