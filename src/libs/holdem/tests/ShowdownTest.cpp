...
namespace
{
    // Pairs nothing and makes no straight or flush on its own.
    // So each seat's hand is exactly what its hole cards make of it.
    ...
    // Aces beat both pairs of kings, which tie with each other.
...
TEST(ShowdownTest, ScoreShowdown_LeavesAFoldedSeatUnscored)
{
    ...
    // The folded seat keeps a default value.
    // So the pots can index it, and it wins no layer with it.
