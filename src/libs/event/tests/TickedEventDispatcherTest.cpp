...
TEST(TickedEventDispatcherTest, Dispatch_ForwardsToWrappedDispatcherAndStampsCurrentTick)
{
    MockEventDispatcher mockDispatcher;
    MockTimedEventSink mockTimedEventSink;
    TickedEventDispatcher tickedEventDispatcher(mockDispatcher, {mockTimedEventSink});
...

    {
        ...
        EXPECT_CALL(mockTimedEventSink, handle(TimedEvent{.tick = 5, .event = mockEvent})).Times(1);
    }

    tickedEventDispatcher.dispatch(mockEvent);
}

TEST(TickedEventDispatcherTest, Dispatch_StampsAdvancingTicksAcrossMultipleCalls)
{
    MockEventDispatcher mockDispatcher;
    MockTimedEventSink mockTimedEventSink;
    TickedEventDispatcher tickedEventDispatcher(mockDispatcher, {mockTimedEventSink});
    ...
    {
        ::testing::InSequence seq;
        EXPECT_CALL(mockTimedEventSink, handle(TimedEvent{.tick = 0, .event = mockEvent1})).Times(1);
        EXPECT_CALL(mockTimedEventSink, handle(TimedEvent{.tick = 1, .event = mockEvent2})).Times(1);
    }
...
TEST(TickedEventDispatcherTest, Dispatch_PropagatesExceptionFromWrappedDispatcherWithoutNotifyingTimedSinks)
{
    MockEventDispatcher mockDispatcher;
    MockTimedEventSink mockTimedEventSink;
    TickedEventDispatcher tickedEventDispatcher(mockDispatcher, {mockTimedEventSink});
    ...
    EXPECT_CALL(mockDispatcher, dispatch(mockEvent)).WillOnce(::testing::Throw(std::runtime_error("mockException")));
