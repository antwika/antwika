    ...
    namespace
    {
            ...
            std::span<const std::uint8_t> bytes, std::size_t at)
        {
            return static_cast<std::uint16_t>(
                (static_cast<std::uint16_t>(bytes[at]) << 8)
                | bytes[at + 1]);
        }
        ...
        [[nodiscard]] std::uint32_t readU32(
            std::span<const std::uint8_t> bytes, std::size_t at)
        {
            return (static_cast<std::uint32_t>(readU16(bytes, at)) << 16)
                | readU16(bytes, at + 2);
        }
            ...
            std::span<const std::uint8_t> bytes, std::size_t at)
        {
            ...
            for (std::size_t index = 0; index < 4; ++index)
            {
                ...
                // A tag is four bytes that need not be printable.
                // This text goes into a message a person reads.
                text.push_back(
                    byte >= 0x20 && byte < 0x7F
        ...
        void requireFlavour(std::span<const std::uint8_t> bytes)
        {
            ...
            {
                throw TtfError(
                    "ttf: this is a font collection rather than a font, "
            ...
            {
                throw TtfError(
                    "ttf: this is an OpenType font with CFF outlines, "
            ...
            {
                throw TtfError(
                    "ttf: these bytes open with '" + tagText(bytes, 0)
        ...
        void requireTablesInside(std::span<const std::uint8_t> bytes)
        {
            ...
            if (tables == 0)
            {
                throw TtfError("ttf: the font declares no tables");
            }
            ...
            for (std::size_t index = 0; index < tables; ++index)
            {
                ...
                // A subtraction rather than offset + length.
                // Both of those are read out of the file being checked.
                // Their sum is the overflow this guard exists for.
                if (offset > bytes.size()
                    || length > bytes.size() - offset)
                {
                    throw TtfError(
                        "ttf: the '" + tagText(bytes, record)
