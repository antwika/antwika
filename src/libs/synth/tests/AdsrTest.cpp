...
// It is also a voice that never quite stops contributing.
TEST(AdsrTest, IsExactlySilentOnceTheReleaseIsDone)
{
    EXPECT_FLOAT_EQ(envelopeAt(kShape, 12, kHold), 0.0F);
    EXPECT_FLOAT_EQ(envelopeAt(kShape, 4096, kHold), 0.0F);
}

// A percussive effect is exactly this.
// Full level immediately, held for a few frames, gone.
TEST(AdsrTest, AnEnvelopeWithNoSegmentsIsFlatWhileItHolds)
{
    constexpr Adsr flat{
...
// Not an error at all.
// It is why the release reads the level it starts from.
TEST(AdsrTest, AVoiceCutOffWhileRisingReleasesFromWhereItGot)
{
    constexpr Adsr slow{
...
TEST(AdsrTest, ComparesFieldByField)
{
    EXPECT_EQ(kShape, kShape);
