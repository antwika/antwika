...
namespace antwika::replay::detail
{
    ...
    // Explicit big-endian encode/decode helpers shared by BinaryEventCodec
    // and the binary replay writer/reader -- never native struct layout, so
    // the format is identical regardless of host byte order.
