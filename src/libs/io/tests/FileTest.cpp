...
namespace
{
    // The caller's own exception type, as every real caller has one.
    // io owns no failure category, so the tests bring theirs along.
...
// Opening is not writing: a full disk fails only on the flush.
// /dev/full is the portable-enough way to force that on purpose.
// This is the tree's one test of that fact.
// Every module that delegates its writes here inherits it.
...
// Bytes mode is what keeps a binary file identical on the way back.
// On MinGW a text-mode stream rewrites every newline it carries.
// The bytes chosen here are the ones that rewrite would corrupt.
