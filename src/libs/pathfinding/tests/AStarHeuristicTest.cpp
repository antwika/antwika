...
namespace
{
    ...
    // 0 -> 1 -> 2 -> 3 at a cost of one a step.
    // A two-node dead end hangs off the start.
...
TEST(AStarHeuristicTest, FindPath_ExpandsOnlyTheRouteWithAnExactHeuristic)
{
    ...
    // The dead end is queued and never reached.
...
TEST(AStarHeuristicTest, FindPath_StillReturnsARouteWhenTheEstimateOverstates)
{
    ...
    // An inadmissible estimate is not an error, and does not hang.
    // It just talks the search out of the cheapest route, costing 2.
    EXPECT_EQ(result.outcome, SearchOutcome::PathFound);
...
TEST(AStarHeuristicTest, FindPath_MayOverpayWhenAnAdmissibleEstimateJumps)
{
    ...
    // Every estimate here is admissible.
    // The one on node 2 is not consistent with the edge leaving it.
    // So node 1 closes at a cost of 5 before the cheaper route lands.
    // A closed node is never reopened.
    // The cheapest route is 0, 2, 1, 3 at a cost of 12.
    EXPECT_EQ(result.outcome, SearchOutcome::PathFound);
