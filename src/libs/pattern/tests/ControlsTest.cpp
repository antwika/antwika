...
TEST(ControlsTest, Ctor_StartsCarryingNothing)
{
...
TEST(ControlsTest, Ctor_CarriesOneValue)
{
...
TEST(ControlsTest, Set_ReplacesAValueAlreadyNamed)
{
...
TEST(ControlsTest, Get_FindsNothingForAnUnknownId)
{
...
TEST(ControlsTest, All_KeepsValuesInIdOrder)
{
    Controls forwards;
...
TEST(ControlsTest, MergedWith_LetsTheOtherSideWin)
{
...
TEST(ControlsTest, MergedWith_KeepsWhatOnlyOneSideCarried)
{
...
TEST(ControlsTest, OperatorEquals_ComparesOnWhatItCarries)
{
    const Controls one(kPitch, ParamValue(60));
...
TEST(ControlsTest, OperatorEquals_ComparesBothControlFields)
{
    ...
    const auto twin = entry;
    EXPECT_EQ(entry, twin);
