...
TEST(NullRendererTest, CreateTexture_ThrowsOnAnIncompleteBitmap)
{
    ...
    // Refused headlessly too, exactly as a real backend would.
    // A bad bitmap must not get further here than there.
    EXPECT_THROW(
        {
...
TEST(NullRendererTest, DrawTexture_AcceptsATextureItDidNotCreate)
{
    ...
    // A real backend declines a texture made somewhere else.
    // This one holds no handle to confuse, so it draws nothing.
    EXPECT_CALL(logger, log(Level::Trace, "gfx.null: draw texture"));
