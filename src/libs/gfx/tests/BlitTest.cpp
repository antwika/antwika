...
{
    // A destination is allowed to hang off the edge, unlike a source.
    // Every backend clips at the window edge the same way.
    EXPECT_TRUE(blitIsDrawable(
        kTexture,
        kWholeTexture,
        Rect{
...
{
    // Added in 32 bits these two come to exactly zero.
    // That would put the source's right edge inside the texture.
    // Nothing but the widening in blitIsDrawable stops it.
