...
namespace
{
    ...
    Vec3 getTransformedPoint(const Transform &transform, Vec3 point)
    {
...
TEST(TransformTest, Matrix_TranslationMovesThePoint)
{
    ...
    EXPECT_TRUE(
        getApproxEqual(getTransformedPoint(transform, {0.0F, 0.0F, 0.0F}),
                    Vec3(1.0F, 2.0F, 3.0F)));
}
...
TEST(TransformTest, Matrix_ScaleMultipliesEachAxis)
{
    ...
    EXPECT_TRUE(
        getApproxEqual(getTransformedPoint(transform, {1.0F, 1.0F, 1.0F}),
                    Vec3(2.0F, 3.0F, 4.0F)));
}
...
TEST(TransformTest, Matrix_RotationAboutZTurnsXTowardsY)
{
    ...
    EXPECT_TRUE(
        getApproxEqual(getTransformedPoint(transform, {1.0F, 0.0F, 0.0F}),
                    Vec3(0.0F, 1.0F, 0.0F)));
}
...
TEST(TransformTest, Matrix_RotationAboutYTurnsZTowardsX)
{
    ...
    EXPECT_TRUE(
        getApproxEqual(getTransformedPoint(transform, {0.0F, 0.0F, 1.0F}),
                    Vec3(1.0F, 0.0F, 0.0F)));
}
...
TEST(TransformTest, Matrix_RotationAboutXTurnsYTowardsZ)
{
    ...
    EXPECT_TRUE(
        getApproxEqual(getTransformedPoint(transform, {0.0F, 1.0F, 0.0F}),
                    Vec3(0.0F, 0.0F, 1.0F)));
}
