...
- `build/bin/antwika_sudoku/antwika_sudoku` — a Sudoku solved by constraint propagation, with `--puzzle` to load one from a file.
...
All of them but `antwika_gfx_demo`, `antwika_gfx3d_demo`, `antwika_sound_demo` and `antwika_sudoku` also take `--record <path>` and `--replay <path>`, which is the replay system below.
