#!/usr/bin/env python3
# Draws the texture atlas apps/poker renders its table from.
# The art is generated rather than painted.
# So a slot is drawn from the same number the C++ addresses it with.
# The layout is read out of antwika/poker/PokerAtlas.hpp.
...
# A 3x5 cell per glyph, in Rank order: deuce first, ace last.
# Drawn white so one blit serves the red suits and the black ones.
RANK_GLYPHS = (
    ("111", "001", "111", "100", "111"),
    ("111", "001", "111", "001", "111"),
    ("101", "101", "111", "001", "001"),
...
def check_layout() -> None:
    """Refuse to draw a layout the header cannot hold."""
...
def unit(value: int, extent: int) -> float:
    """Put a pixel centre on a 0..1 axis across a slot."""
    return (value + 0.5) / extent


def rounded(east: float, south: float, inset: float, radius: float) -> bool:
    """Whether a point is inside a rounded rectangle inset from the cell."""
    left = inset
    right = 1.0 - inset
...
def noise(px: int, py: int, spread: int) -> int:
    """A repeatable per-pixel wobble, so the felt is not flat."""
...
def card_back_pixel(px: int, py: int) -> Rgba:
    ...
    # A lattice, so a face-down card reads as patterned rather than flat.
...
def chair_pixel(px: int, py: int) -> Rgba:
    ...
    # A back rest above a seat pad, which is enough to read as a chair.
...
def chip_pixel(px: int, py: int) -> Rgba:
    ...
    if not disc(east, south, 0.34):
        # Six notches around the rim, the way a chip is edge-spotted.
...
def dealer_pixel(px: int, py: int) -> Rgba:
    ...
    # A bar and a bowl, which at this size is as much of a D as fits.
...
def to_act_pixel(px: int, py: int) -> Rgba:
    ...
    # A triangle pointing down at whoever the hand is waiting on.
...
def suit_pixel(px: int, py: int, suit: int) -> Rgba:
    ...
    if suit == 1:
        # Diamonds: a square stood on its corner.
    ...
    if suit == 3:
        # Spades: the heart upside down, on a stem.
...
def rank_pixel(px: int, py: int, rank: int) -> Rgba:
    ...
    # Centred in the cell, at whatever whole scale the cell allows.
