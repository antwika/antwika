...
def it_detects_two_sentences_sharing_a_line() -> None:
    text = "First sentence. Second sentence."
    assert m.has_multiple_sentences(text) is True


def it_allows_a_single_long_sentence() -> None:
    text = (
...
def it_does_not_treat_abbreviations_as_sentence_ends() -> None:
    text = "Uses a mock, e.g. MockFoo, for isolation."
    assert m.has_multiple_sentences(text) is False


def it_does_not_treat_a_dotted_identifier_in_a_code_span_as_a_break() -> None:
    text = "See `Foo.Bar` for details."
    assert m.has_multiple_sentences(text) is False


def it_ignores_a_leading_numbered_list_marker() -> None:
    assert m.has_multiple_sentences("1. Do the first thing.") is False


def it_treats_a_period_followed_by_space_and_capital_as_a_break() -> None:
    assert m.ends_sentence("This line ends properly.") is True


def it_treats_a_missing_terminator_as_not_ending() -> None:
    assert m.ends_sentence("this line just trails off") is False


def it_ignores_trailing_decoration_when_checking_the_terminator() -> None:
    assert m.ends_sentence("**Bold sentence.**") is True


def it_recognizes_a_badge_only_line_as_pure_markup() -> None:
    text = (
...
def it_does_not_treat_a_sentence_containing_a_link_as_pure_markup() -> None:
    text = "See [the docs](https://example.com) for details."
    assert m.is_pure_markup(text) is False


def it_flags_a_single_comment_line_with_two_sentences() -> None:
...
def it_flags_a_sentence_wrapped_across_two_comment_lines() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "Foo.cpp"
        write(
            path,
...
def it_allows_a_properly_split_two_sentence_comment_block() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "Foo.cpp"
...
def it_does_not_flag_an_inline_trailing_comment_fragment() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "Foo.cpp"
...
def it_does_not_follow_a_url_scheme_into_a_comment() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "Foo.cpp"
...
def it_skips_over_a_hash_bang_that_is_not_a_real_comment_start() -> None:
...
def it_flags_a_sentence_wrapped_across_two_markdown_lines() -> None:
    with tempfile.TemporaryDirectory() as tmp:
...
def it_does_not_flag_consecutive_short_list_items() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "doc.md"
        write(
            path,
...
def it_does_not_flag_a_multi_line_badge_block() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "doc.md"
        write(
            path,
...
def it_does_not_scan_inside_fenced_code_blocks() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "doc.md"
        write(path, "```sh\nsome command\nspread across lines\n```\n")
...
def it_does_not_scan_markdown_table_rows() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "doc.md"
        write(
            path,
...
def it_does_not_flag_a_table_row_as_a_wrapped_sentence() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "doc.md"
        write(
            path,
...
def it_checks_the_readme_and_no_other_markdown() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write(root / "wiki/foo.md", "A third wrap\nonto a second line.\n")
...
def _comment_texts(path: Path, marker: str) -> list[tuple[int, str]]:
    texts = []
...
def it_keeps_every_comment_at_or_under_eighty_characters() -> None:
    root = m.DEFAULT_ROOT
    too_long = []
...
def main() -> None:
    tests = [
        ...
        it_does_not_treat_a_dotted_identifier_in_a_code_span_as_a_break,
