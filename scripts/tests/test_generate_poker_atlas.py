...
def opaque_pixels(pixels: bytearray, width: int, slot: int) -> int:
...
def it_draws_an_atlas_the_header_can_address() -> None:
...
def it_draws_something_in_every_slot_the_app_addresses() -> None:
...
def it_draws_a_distinct_glyph_for_every_rank() -> None:
    width, _, pixels = generate_poker_atlas.build_atlas()
...
def it_covers_the_felt_tile_edge_to_edge() -> None:
    width, _, pixels = generate_poker_atlas.build_atlas()
...
def it_leaves_the_glyph_slots_white_so_a_tint_decides_the_colour() -> None:
    width, _, pixels = generate_poker_atlas.build_atlas()
...
def it_refuses_a_slot_two_painters_share_or_one_off_the_atlas() -> None:
...
def it_writes_a_png_and_then_reports_it_as_current() -> None:
...
def it_reports_a_missing_atlas() -> None:
    with tempfile.TemporaryDirectory() as directory:
        assert run_main("--root", directory, "--check") == 1


def it_reports_a_stale_atlas() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
...
def it_keeps_the_committed_atlas_in_step_with_the_generator() -> None:
