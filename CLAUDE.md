...
- `apps/game` is an isometric grid you build on with the mouse: left-click lays a path tile, left-drag lays a whole run of them, right-click drops a walker onto one, middle-drag pans and the wheel zooms, and F10 opens a menu modal over the city.
  ...
  **A drag holds nothing still**, so a route is planned against a city that goes on moving under it between the press and the release.
  It used to pause the run for exactly that reason, and a city now runs all the time unless a player has asked otherwise, so the bookkeeping about whose pause a drag was letting go of is gone with it.
  ...
  **A route that does not exist builds nothing at all**: the preview shows the two cells that were named, reddened, following `canPlace()`'s convention that a refusal shown is one somebody can act on, and half a route would be a road to nowhere nobody asked for.
  A release is exempt from the toolbar's covering rule for the reason a movement is plus one of its own: a drag let go over the bar would otherwise lay no road at all.
  ...
  **Opening it holds nothing**, and neither does entering a city: the city goes on behind the modal exactly as it goes on behind the world map, since a run is held only where a player has asked for it.
  ...
  **The pause is `apps/life`'s answer to the same question**: `game::PauseGatedSystem` wraps a system and stages nothing while `game::PauseState` says the run is held, exactly as `life::DragPausedSystem` does, and only those systems stop.
  ...
  It composes with `game::SessionGatedSystem` rather than replacing it, since a run is paused *and* has a session on screen and either gate alone answers only its own question.
  That gate asks `game::simulates()` rather than naming one mode, which is what lets a city go on running while whoever is playing it reads the world map; it stops only on the two screens where there is no session to run, the main menu and the save picker.
  ...
  **A pause is asked for, and nothing else causes one.**
  A city coming up, the menu modal opening and a road being dragged out each used to hold the run as well, and none of them does: a city progresses all the time until somebody presses the bar's pause button, which is also the one way back out.
  That deleted `game::CityEntrySink` outright and left `PauseState` one writer rather than four, and what it buys beyond the simplification is that opening a screen is no longer a pause nobody asked for -- which matters most where more than one player shares a city, since one of them reading the map must not stop it for the others.
  **The value is absolute rather than a toggle**, which is the same decision read forward: `PauseState::set(bool)` is idempotent and the button sends the opposite of the state it was showing, so two players pausing on one tick agree on a pause where two toggles would have cancelled out and left the run going with both of them having watched themselves press pause.
