...
- **Anything the meaning of a click depends on is simulation state**, which is why `apps/game`'s camera, `apps/atlas_editor`'s `CanvasView` and `apps/music_editor`'s scrolled-to line are, and why a zoom is an index into a table of whole sizes and a scroll is a line number rather than a pixel offset (see [`blog/013-the-camera-is-simulation-state.md`](blog/013-the-camera-is-simulation-state.md)).
...
- [`config`](wiki/libraries/config.md) is the versioned-JSON plumbing every app's config file shares -- the magic/version envelope, the bounded-number shapes, the migrate-then-validate pipeline and the file handling -- while each app keeps its own magic, fields, defaults and decode.
- [`console`](wiki/libraries/console.md) is the tilde-toggled debug console every app mounts and the `dump_state`/`load_state` seam behind it -- the shell, the gating and the command policy stated once, while each app keeps its own snapshot store, magic and migrations; it defines no event, and `load_state` runs only in a plain live run.
...
- [`ecs_commons`](wiki/libraries/ecs_commons.md) is the vocabulary half of that, header-only and deliberately small -- `Name`, and the `GatedSystem` decorator each app's pause/mode/drag gate forwards to.
  `GridPosition`, `Velocity`, `Lifetime`, `Tag` and the three systems over them were pruned once it was clear no application had ever called one.
...
- [`geometry`](wiki/libraries/geometry.md) is `Point`, `Size` and `Rect`, header-only and depending on nothing -- they moved out of `gfx` so a module wanting a width and a height does not link a graphics library to get one; `gfx::Size` still names `geometry::Size` through a re-export, so no caller had to change.
...
- [`i18n`](wiki/libraries/i18n.md) is a compiled-in catalogue keyed by a symbolic message id rather than by the English string, with a total lookup that never throws; a `Translator` is injected like any other collaborator, and an application that hit-tests a layout either fixes its locale in `main()` or makes it simulation state outright -- never reads one from a flag or the environment, since neither is carried by a recording.
  [`game`](wiki/apps/game.md) is the one that made it simulation state, so a player can pick a language while playing: `LocaleState` folds it in the tick path and stages a change to the tick boundary, and only the locale a run *starts* in is announced onto the wire.
...
- [`pattern`](wiki/libraries/pattern.md) is a pattern algebra over exact rational cycles in TidalCycles' and Strudel's shape, where a pattern is a pure function from a window of time to the events in it -- and it knows nothing about music, since an event carries caller-named `Controls` rather than a note, exactly as `wfc` knows nothing about grids.
- [`notation`](wiki/libraries/notation.md) reads the Tidal mini-notation -- `"0 [3 5] <7 9>"` -- into that algebra and owns no semantics of its own, so a string that parses but asks for nine onsets across eight steps is refused by `pattern` rather than by the grammar.
- [`sequencer`](wiki/libraries/sequencer.md) is where musical time meets frame time and the only place the two meet: it advances a half-open window in *cycle* space once per tick and triggers on onsets alone, since a held note yields a fragment in every window it spans and sounding those restarts it at every boundary.
