...
- **Running a run and recording one are two libraries**: `antwika::simulation` is the loop, the `ITickSource` seam it reads through, `TickPacer` and `WindowInputSource`, and `antwika::replay` is the file format, its migrations, the `--record`/`--replay` flags and the `ReplaySource` that feeds a loaded document back in.
...
- `antwika::simulation::EngineLoop` is the one code path shared by live and replay runs: each tick it asks an `ITickSource` for that tick's events, dispatches them through a `TickedEventDispatcher`, then steps the engine.
  Live vs. replay differ **only** in what implements `ITickSource` — a script feeding `ReplaySource` from a loaded file, vs. a live/hand-authored input source.
...
- `apps/poker` is a no-limit Texas hold'em cash game on top of a new `antwika::holdem` library.
  ...
  **There is one layout, and the art is placed from it**: `describe()` names a container for every card, seat and badge (`poker::widgets` in `TableWidgets.hpp`), and `describeArt()` is handed that frame's `ui::WidgetRects` and blits into the rectangles it reports -- an id the frame did not declare draws nothing, since inventing a rectangle for it is the second layout this deletes.
  ...
  The only route back in is `poker::WindowCloseSource`, an `ITickSource` decorator that appends `engine.stop` once the window has gone, so a close is ordinary replay input and lands in a `--record` file like anything else.
...
`antwika::gfx` is nevertheless *on* that link line, transitively: `antwika::simulation` links `antwika::ecs` and `antwika::gfx` for `TickPacer` and `WindowInputSource`, and `antwika::input` links it because every source it offers is an `ITickSource`.
...
Live input reaches the engine only through `ITickSource`, via `LiveInputSource`, and is persisted by `InputEventCodec` as `input.*` events with symbolic key and button names rather than platform scancodes, so a session recorded under one backend replays under another.
**What lands in a recording is decided by where the recorder sits rather than by a list of names it skips.**
`event::TickEventRecorder` is an ordinary `ITickEventSink` an app registers on its `TickedEventDispatcher`, and it records unconditionally, so what it sees is exactly what an `ITickSource` supplied for that tick -- never `engine.tick`, which `Engine::step()` dispatches for itself after `EngineLoop` has drained the source, and never anything a sink derives further down the tick path, like the tile `game::GridSink` lays from a click.
