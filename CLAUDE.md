...
build/bin/antwika_companion/antwika_companion            # feed, play, bed
...
`antwika_companion` opens a 256x256 window holding one animal whose **energy is its life**, and three props to press: a bowl to feed it, a ball to play with it, and a nest to put it to bed.
Playing is what spends the energy and what keeps it happy, happiness is the rate the energy runs down at, and a bed is refused until it is tired enough to have earned one -- so the game is to burn as close to nothing as you dare and then put it to bed before it drops.
Running out is a *collapse*, which costs a slice of its energy ceiling for good, and the ceiling running out is the only way one ever perishes.
...
- `apps/companion` is a tamagotchi in a 256-pixel window, and its whole design is that **energy is the life meter and happiness is the rate on it**.
  Energy drains while the companion is awake, at a period one of four happiness bands picks out, with a famished companion and a bored one each paying a second drain on the same beat.
  Energy reaching nothing is a **collapse** rather than an end: it drops where it stands, falls asleep, and loses `companion::kCollapsePenalty` off its energy *ceiling* for good.
  **The ceiling running out is the only way one ever perishes**, so starving, boredom and misery kill nothing directly and only bring a collapse forward -- the same "every violation meets in one place" rule this app always had, read out over a resource somebody can watch shrink.
  `companion::Pet` is the simulation -- integer throughout, no clock and no generator, so it is a pure function of how many times `step()` has been called and which verb was called between them.
  **Sleep ends when the energy is back rather than at an hour, and bedtime is refused above `kTiredPercent` of the ceiling**, which is the rule that stops sleeping being free: a companion has to spend the day near the edge before it is allowed to recover from it.
  That costs the invariant version 1 had, `night(t) = (t % (dayTicks + nightTicks)) >= dayTicks`, and it is spent knowingly -- both dusk and dawn are stored state now, and `Pet::night()` is a state rather than an hour.
  **The app defines no event for feeding, playing, putting to bed or prodding it**: a press is the input, `companion::PropSink` asks `propAt()` which of the three props it landed on and calls that verb inside the tick path, and the replay stores the press and regenerates the rest.
  So `companion::PetLayout.hpp` is this app's `life::BoardLayout` in earnest, with `propBox()` shared by the `PetScene` that paints the props and the `PropSink` that hit-tests them, against the *configured* window size and never the reported one.
  A press on none of the props is `Pet::pester()` rather than nothing at all, so sloppy aim has a price and the props stay worth aiming at; a press of any kind while it is asleep wakes it, and that rule lives in `Pet` rather than in the sink, since the sink says where a press landed and the companion says what it was in a state to do about it.
  **`Saying::Yawn` is load-bearing rather than decoration**: bedtime is refused above the tired mark, so a player with no way of knowing when that window opened would be playing against a hidden rule rather than a hard one.
  **How grown up it is is `stageAt()` over its tick count and nothing else**, and what a stage decides is how much energy it may hold -- growing up is capacity here, so the gauge and the picture tell one story.
  The ceiling is therefore stored nowhere: it is `baseEnergyFor(stage)` less `kCollapsePenalty` per collapse, floored at nothing, which is arithmetic over two numbers the companion already keeps and so has no third copy a file could put out of step.
  An elder holds a child's ceiling rather than an adult's, so **old age can be what finally ends a companion that has collapsed often**, with no further collapse needed.
  `companion::PetForm` is what it grew into, decided by `formFor()` over the care record alone and reaching the picture as a shade on the one fur each palette holds.
  `companion::moodOn(day)` gives each day a `DayMood` that hurries exactly one period along by a third, hashed from the day's own number in the idle chatter's sense and for its reason; day zero is deliberately `Ordinary`, which is what keeps every trace in `PetTest` readable.
  **`ReviveSink` defines no event either**, and it is registered *after* `PropSink` so one press can never be a prop and a revival both.
  It is also where the lineage is written, because `Pet::revive()` replaces the companion whole: the ending age is offered to the record before the replacement and the generation moves on after, since afterwards there is nothing left to ask how long it lived.
  Rendering is a write-only projection in structure rather than by promise: `companion::snapshotOf()` takes an immutable `companion::PetSnapshot`, `companion::PetScene` turns that into drawing calls, and `companion::RenderSink` runs it once per `engine.tick`.
  **The animal is drawn from `IRenderer`'s rectangles rather than blitted from an atlas**, which is a choice rather than a shortcut: ten boxes at 256 pixels square are shorter written down than described, and a scene with no art in it stays assertable against a mock renderer call by call.
  Four gauges take the top of the window -- energy, hunger, fun and happiness -- and **the energy gauge is the one whose own end moves**, its track drawn to the ceiling the companion still has so a collapse shortens the bar rather than merely its contents.
  The three props are painted into the very boxes `propAt()` hit-tests, and the one the companion would like is lit rather than merely present, which is this app's whole answer to instructions.
  Three lines of text report the run in words -- what it is doing, then `d3 teen heavy`, then `gen 2 best 900` -- every character of them read off the snapshot.
  **A speech bubble appears beside it from time to time**, holding one of fifteen lines; which line comes up and when are `Pet`'s decisions rather than the scene's, so a replay says the same words on the same ticks, and the idle line is a hash of the tick rather than a draw from an `antwika::rng` generator.
  ...
  `companion::PacingSink` is the `ITickEventSink` shape over `simulation::TickPacer`, registered last so the order is present-then-wait.
  **A live session carries its companion over from the one before it, and a replay neither reads nor writes one.**
  `companion::CompanionMemory` is what a file holds: a `PetMemory` -- everything the simulation holds, taken with `Pet::remember()` and put back through `Pet`'s restoring constructor -- and a `LineageMemory`, which is kept apart precisely because `revive()` replaces the companion whole and the record has to outlive that.
  Neither the ceiling nor the stage is persisted, since both are derived, and no configuration is either, since which numbers a companion is balanced with is this build's decision rather than a file's.
  It is a versioned JSON document read as `parse -> read version -> migrate -> validate -> decode` exactly as `apps/game`'s save is, at `companion::kSaveFormatVersion` 3, with `standardPetMigrations()` carrying a version 1 companion across by keeping what still means the same thing and starting the rest where a new companion starts.
  ...
  Three rules cover the awkward cases: it is written **once, after the loop has finished**; a **replay** is offered no store at all by `storeIfLive()`; and a **file that will not read** is logged and stepped over rather than fatal, with only a failure to *write* thrown, as `companion::SaveFormatError`.
...
**Two applications call it, and they are the only two with anything to tween**: a tween needs a value that moves *and* more than one picture drawn while it moves, and only `apps/game` draws more than one frame a tick while only `apps/companion` runs a multi-tick animation it can resolve a position out of.
`apps/companion` breathes on `QuadInOut` through `kBreatheEasing`, which is what the library was worth building for -- a linear breath reads mechanical, a sine one is what a breath wants and is not exact, so `QuadInOut` is the closest curve that stays in whole numbers.
`apps/game` draws a walker's slide through `game::kWalkerEasing`, and that constant is `Easing::Linear` on purpose rather than pending: a walker crosses many cells in a row, so easing each step would make it lurch at every tile rather than walk.
