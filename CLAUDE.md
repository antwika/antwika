...
`antwika::ttf` turns the bytes of a TrueType font into metrics and coverage masks: `Font`/`TtfReader` parse one, `FontMetrics`/`GlyphMetrics` say what it holds, `rasterise()` draws one glyph into a `Coverage`, and `makeGlyphAtlas()` packs a set of characters into one mask plus the map from a character to the `Rect` holding it.
**It depends on no other module of this project**, which is the decision the rest follows from: a glyph is one channel rather than four, so the output is `ttf::Coverage` rather than `gfx::Bitmap` -- reusing that struct would have taken on GLM and `antwika::log` for three fields, where expanding a mask into white RGBA with the coverage in alpha is four lines at the one seam that wants a texture, and leaves the tint deciding the colour.
`TtfReader::read()` takes a `std::istream` and `Font`'s constructor takes bytes, for `PngReader`'s and `WavReader`'s two reasons: the library opens no files, and every refusal is therefore reachable from bytes in memory -- which is what makes the coverage gate satisfiable here, since `src/libs/ttf/tests/SyntheticFont.cpp` builds a whole valid four-glyph font in memory and **no font binary is checked in at all**.
...
See [`wiki/libraries/ttf.md`](wiki/libraries/ttf.md).
