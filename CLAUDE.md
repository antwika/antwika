...
- [`music_editor`](wiki/apps/music_editor.md) is a live-coding editor: one pane of mini-notation, always sounding, where every keystroke reaches the music with nothing reloaded -- a voice line opens with `$:` and *names* its instrument rather than being counted into one, a line that will not parse keeps playing whatever it last did, and pausing stops the musical clock rather than the device.
