...
The shared `antwika::network::tests::conformance` suite runs against both, and a backend that cannot do something skips on `NetworkCapabilities` rather than failing -- a backend pretending to connect so a test goes green is exactly the dishonesty a conformance suite exists to prevent.
