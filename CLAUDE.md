...
- `apps/game` is an isometric grid you build on with the mouse: left-click lays a path tile, right-click drops a walker onto one, middle-drag pans and the wheel zooms.
  ...
  **A right press means one of two things, and the palette decides which**: while a building tool is selected it leaves build mode, putting `BuildTool::Road` back and placing nothing by that press, and otherwise it drops a walker on the path under the pointer exactly as it always did.
  Road is "normal play" here because it is the tool a fresh session holds and the one a cancel returns to, so cancelling twice is cancelling once and the palette is never left showing a tool no button holds down.
  The whole decision is `GridSink`'s rather than split with `UiSink`: `UiSink` runs first, so a cancel resolved there would leave the road tool selected by the time the grid read that same press, and one press would then cancel *and* drop a walker.
