...
- `apps/game` is an isometric grid you build on with the mouse: left-click lays a path tile, right-click drops a walker onto one, middle-drag pans and the wheel zooms.
  ...
  **What the first second of a run shows is a placeholder rather than a number**: `FrameMeter::perSecond()` answers nothing at all until a whole window has gone by, and `describeFps()` draws `game::kNoRateYet` -- two dashes -- in its place.
