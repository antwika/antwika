...
See [`wiki/apps/atlas_editor.md`](wiki/apps/atlas_editor.md).
...
- **Running a run and recording one are two libraries**: `antwika::simulation` is the loop, the `ITickEventSource` seam it reads through, `TickPacer` and `WindowInputSource`, and `antwika::replay` is the file format, its migrations, the `--record`/`--replay` flags and the `ReplaySource` that feeds a loaded document back in.
  `replay` depends on `simulation` and never the reverse, so a live run links no JSON, no schema validator and no migration chain.
  Both were one library called `replay`, which named the smaller half of it -- a live run is not a replay at all -- and left the seam every live input source implements called `IReplaySource`.
- `antwika::simulation::EngineLoop` is the one code path shared by live and replay runs: each tick it asks an `ITickEventSource` for that tick's events, dispatches them through a `TickedEventDispatcher`, then steps the engine.
  Live vs. replay differ **only** in what implements `ITickEventSource` — a script feeding `ReplaySource` from a loaded file, vs. a live/hand-authored input source.
  This is what makes replay reproduce state by construction, not by convention.
  The seam is named for what its one method does rather than for one of its implementers: `input::LiveInputSource`, `input::IdleMotionSource`, `input::PointerHintSource`, `app::FramePacedSource` and three applications' window sources implement it, and none of them touches a recording.
...
- `replay::ReplayFormatError` / `simulation::EngineLoopError` are the specific exception types for bad replay input and loop misuse respectively, following the project's one-exception-type-per-failure-category rule.
...
- `apps/game` is an isometric grid you build on with the mouse: left-click lays a path tile, left-drag lays a whole run of them, right-click drops a walker onto one, middle-drag pans and the wheel zooms, and F10 opens a menu modal over the city.
  ...
  **The block a click would take is outlined as well as tinted**, by `game::footprintOutline()` -- four points traced round the very `footprintBounds()` box the ghost tile is blitted into, so a border and the preview it surrounds cannot show two different extents.
  ...
  **A road is dragged out rather than clicked one cell at a time, and that is no more an event than one click is.**
  What a recording holds is the press, the movements and the release; `game::RoadDrag` is where the gesture's start and end live, and `game::planRoad()` is what says how the one gets to the other.
  That plan is an A* through `antwika::pathfinding` on exactly `stepTowards()`'s terms -- ties break down to ascending `NodeId`, and the extent is passed in rather than derived from what happens to exist, since a bounding box taken off the roads would renumber every node as one was laid and the tie-break with it.
  ...
  `RoadDrag` is simulation state in the camera's sense -- written by `GridSink` inside the tick path, never from `input::PointerHintChannel` -- and the preview rides on `SceneSnapshot::plan`, which unlike the ghost beside it is filled in once a tick rather than once a frame, since there is nothing about it a frame could see that the tick did not.
  **The pressed cell is laid at once rather than at the release**, so a plain click stays the single-tile placement it always was and a recording holding no release lays exactly what it always laid -- which is every recording written before this, `src/apps/game/replays/demo.json` included.
  Three decisions are worth stating outright.
  **Only roads are dragged**: a run of houses is not a route, so a path search says nothing about where one would go, and a building tool would need a rule of its own about what a rectangle of blocks means -- so a building is still placed on the press and the press alone.
  **A drag holds the run still and lets it go again**, so a route cannot be planned against a city moving under it; but it releases the pause only when the drag was what held it (`RoadDrag::heldForDrag()`), so a drag never resumes a run somebody paused for themselves.
  ...
  **A route that does not exist builds nothing at all**: the preview shows the two cells that were named, reddened, following `canPlace()`'s convention that a refusal shown is one somebody can act on, and half a route would be a road to nowhere nobody asked for.
  A release is exempt from the toolbar's covering rule for the reason a movement is plus one of its own: a drag let go over the bar would otherwise hold the pause for the rest of the session.
  **A right press means one of two things, and the palette decides which**: while a building tool is selected it leaves build mode, putting the palette *down* and placing nothing by that press, and otherwise it drops a walker on the path under the pointer exactly as it always did.
  So `game::UiOverlay::tool()` is a `std::optional<BuildTool>`, and nothing selected is a state the app can be in rather than a synonym for the road tool: no button on the bar is held down, `ghostFor()` returns an invisible ghost so neither a preview tile nor its `footprintOutline()` border is drawn, and a left press lays nothing at all.
  It used to fall back to `BuildTool::Road`, which made cancelling twice the same as cancelling once at the price of a cancel that quietly armed a different tool; putting the palette down says what a cancel means, and a right press with nothing selected still drops a walker, so cancelling twice is still cancelling once.
  ...
  **The picture is hand-drawn art and is the source of truth for how the game looks**; it used to be generated by a script CI re-ran to prove the committed PNG had not drifted, and that script is gone, so nothing rebuilds the atlas and editing it is editing the art.
  ...
  What is left to catch a mistake is the `static_assert`s in that header, `TileAtlasTest`, and a check at startup that the PNG really is `kAtlasSize`; see [`wiki/apps/game-texture-atlas.md`](wiki/apps/game-texture-atlas.md) for what an artist has to produce.
  A toolbar of zoom, reset-view, pause and menu buttons is drawn over the grid by `game::Toolbar`, described and resolved once per tick by `game::UiSink` -- registered *before* `GridSink`, so a press is resolved against the bar before the grid sees it -- and painted last by `RenderSystem`.
  It defines no event either, for the same reason: what a recording holds is the click.
  **The menu modal is a modal rather than a mode, and whether it is up is simulation state** in exactly the sense `PauseState`, the camera and the selected tool are.
  F10 -- the symbolic `input::Key`, so a session recorded under one backend opens it under another -- and the bar's `menu` button both open it, `UiSink` owns the flag because it is the only thing that reads one, and no `ui.*` or `game.*` event exists for opening, closing or leaving by it.
  `game::MenuModalScene` is the picture, and its scrim is load-bearing rather than decoration: a container the size of the whole canvas with a fill behind it, so `ui::Interactions::pointerOverUi` is true wherever the pointer is and `GridSink`'s existing "what the bar covers, it covers from the grid too" rule keeps every press off the city with no second mechanism invented for it.
  Its commands are appended after the bar's in the one `UiOverlay` the renderer paints last, which is how "on top" is said where `antwika::gfx` offers no depth but paint order, and a press is resolved against the modal alone so a toolbar button cannot be pressed through it.
  Three awkward cases have rules, and each is stated in the code as well.
  **Opening it ends a road drag in progress, and that drag lays nothing at all**: what a drag lays is what its release said, and a release arriving over the modal never said it -- so `UiSink` calls `RoadDrag::finish()` rather than `GridSink`'s cancel, which may resume.
  **Opening it holds the run**, exactly as `CityEntrySink` does on entering a city, `hold()` rather than `toggle()` for that sink's reason, and closing it does not let the run go again, so the way out is the pause button it always was -- which also settles the drag, since the modal's hold supersedes whatever the drag was holding.
  **Leaving for the main menu is a mode change like every other**, asked for on `AppModeState` so it lands at the tick boundary, and the modal is put away on the way out so a city entered later is not still wearing it.
  Its widget ids live in `modalWidgets` rather than in the bar's `widgets`, for the reason `menuWidgets` has a namespace of its own: the two are resolved against different frames and never share one.
  ...
  **The frames drawn between two ticks carry on as well, which is why the pause reaches the picture too**: `game::SceneSnapshot` carries it, read off `PauseState` by `snapshotOf()` exactly as the camera is, and `game::GridScene` then draws a held walker at its step's own phase whatever fraction of a tick a frame falls at.
  Without that a walker frozen mid-step slid forward through every tick and snapped back at the start of the next one, for as long as the run stayed held -- the whole ticks of its step stop with `WalkerSystem`, and `app::FramePacedSource`'s frames do not stop with them.
  ...
  **A city is entered paused**, so progress is something a player asks for rather than something that starts happening at them the moment a grid appears, and the way out is the same toolbar button it always was.
  `game::CityEntrySink` is what does it: registered immediately after `AppModeState`, which commits the mode, and ahead of `GridSink`, which runs the systems a pause stops.
  ...
  **What the first second of a run shows is a placeholder rather than a number**: `FrameMeter::perSecond()` answers nothing at all until a whole window has gone by, and `describeFps()` draws `game::kNoRateReadout`, which reads `fps --`, in its place.
That constant is the whole line rather than the two dashes alone, and the readout is two `ui.label()` calls rather than one over a conditional string, because a caption joined to a placeholder is a `std::string` built on a branch -- a temporary whose unwind path no test can reach, and two branches the coverage gate then refuses.
  ...
  **What a building depends on and what a walker carries are drawn as small vertical bars, and hovering either says the same thing in words.**
  `game::BuildingSprite` is `BuildingView`'s render-side twin for exactly `WalkerSprite`'s reason: it carries the stock a gauge is drawn from, and `GameSummary` keeps the plain view, so a number that exists only to be looked at stays out of the value `ReplayDeterminismTest` compares -- `buildingViewsOf()` is what a summary is built from now, and `WalkerSprite` gained the walker's kind and load on the same terms.
  A bar is a track and a fill in `game::ResourceBar.hpp`, worked out from `footprintBounds()` and `walkerBounds()` -- the very boxes the sprite is blitted into -- so the gauges cannot become a second layout that drifts from the art, and they are painted in a pass *after* every sprite so nothing standing in front of what they gauge can hide one.
  ...
  Hovering is `game::hoverFor()`, and it is `ghostFor()`'s sibling in every way that matters: it reads `input::PointerHintChannel`, it resolves the pixel through the same `screenToCell()` a click goes through, it tests a building across its whole block, and what it answers may decide what is drawn and nothing else.
...
- `apps/poker` is a no-limit Texas hold'em cash game on top of a new `antwika::holdem` library.
  ...
  **The seats are a ring around the table rather than a list down the side of it**, and that ring is the same one layout: three bands -- the top seats, then a row of the left seats, the table and the right seats, then the bottom seats -- with `ringPlan()` deciding which seat lands on which side from the seat count alone, the two short sides taking a quarter each and the long ones sharing the rest, since a table is wider than it is tall and so is a seat.
  Each seat is one fixed size wherever it landed and carries its own name, stack, status line, stack bar, badges and hole cards, so nothing about the picture depends on how many places the table has; the bottom and left sides run against seat order, which is what carries the ring round clockwise.
  What a seat last did is a line of words -- `bet 40`, `all in`, `folded`, `to act`, `waiting`, in that order of precedence -- and it is **derived from the `SeatSnapshot` rather than remembered**, so no history had to enter simulation state and a replay reaches the same words from the same state.
  The felt they sit around is one atlas slot (`kTableSlot`) stretched into the rectangle the layout gave the middle of the table, because `IRenderer` has no rounded-rectangle call and no clipping: the rounding is art, and a stack of fills per corner would have been a second geometry beside the layout's.
  ...
  The only route back in is `poker::WindowCloseSource`, an `ITickEventSource` decorator that appends `engine.stop` once the window has gone, so a close is ordinary replay input and lands in a `--record` file like anything else.
...
- `apps/companion` is a tamagotchi in a 256-pixel window, and its whole design is that **when a tap lands is what decides what it means**: a meal while the companion is awake and hungry, an interruption while it is asleep, and nothing at all otherwise.
  ...
  **The app defines no event for feeding it**: a press is the input, `companion::TapSink` turns it into a meal or a rude awakening inside the tick path, and the replay stores the press and regenerates which it was.
  A press anywhere means a tap, and the one press whose meaning depends on *where* it landed is the "new pet" button a perished companion is offered -- so `companion::PetLayout.hpp` is this app's `life::BoardLayout`, with `reviveButtonRect()` shared by the `PetScene` that paints it and the `ReviveSink` that hit-tests it, against the *configured* window size and never the reported one.
  **`ReviveSink` defines no event either**: a press is the input, it turns one into a new companion inside the tick path, and it is registered *after* `TapSink` so one press can never be a tap and a revival both -- the other order would start a companion and immediately feed it.
  `Pet::revive()` is the constructor's own idea of a new companion rather than a second list of starting values, and it is legal at any time, since which press means "start again" is the sink's decision and a rule enforced in two places can be enforced differently in each.
  ...
  **A speech bubble appears beside it from time to time**, holding one of a fixed set of lines: four are idle chatter, and five say something about the run -- `feed me!` while it is hungry, `yum yum!` for a meal, `im full!` for one it did not want, `shhh!` for being woken and `zzz...` while it sleeps undisturbed.
  Which line comes up and when are `companion::Pet`'s decisions rather than the scene's, so a replay says the same words on the same ticks: a tap is answered on the tick it lands, chatter comes up every `chatterPeriodTicks`, a line already up runs its course before another may start, and `lose()` clears the bubble as it perishes so nothing is ever said over a grave.
  The idle line is a hash of the tick rather than a draw from an `antwika::rng` generator -- a generator here would be a seed and a stream position for a save file to carry and keep in step with the ticks, and one let out of step would say the wrong thing for the rest of the session, where a hash of a number `Pet` already holds is nothing at all to carry.
  The snapshot carries a `companion::Saying` and neither the words nor the countdown: the words are one table in `PetScene.cpp` beside the readout's own, and a scene handed a countdown could start counting one down itself.
  ...
  **A live session carries its companion over from the one before it, and a replay neither reads nor writes one.**
  `companion::PetMemory` is everything the simulation holds -- the ticks, the state, both needs, the three counts, the disturbed note and both speech-bubble fields -- taken with `Pet::remember()` and put back through `Pet`'s restoring constructor, and it carries no configuration, since which numbers a companion is balanced with is this build's decision rather than a file's.
  It is a versioned JSON document read as `parse -> read version -> migrate -> validate -> decode` exactly as `apps/game`'s save is, at `companion::kSaveFormatVersion` 1 in the shared `"version"` member, with `standardPetMigrations()` its own injected `replay::MigrationChain` and `kSaveMagic` telling it apart from the other two formats that state their version the same way.
  `companion::IPetStore` is the one seam to a filesystem in `atlas_editor::IAtlasStore`'s shape, `FilePetStore` its one implementation, and `PetSave.hpp` the format alone -- so a round trip is assertable with no file on disk.
  Three rules cover the awkward cases: it is written **once, after the loop has finished** rather than every tick or on a timer, so a run ended with `Ctrl+C` keeps nothing exactly as a `--record` run there writes no file; a **replay** is offered no store at all by `storeIfLive()`, since a replay resuming from whatever is on the machine running it would reach a different session and saving would overwrite a live companion with a regenerated one; and a **file that will not read** is logged and stepped over rather than fatal -- a missing one is an ordinary first run, a corrupt one starts a new companion and is written over, and only a failure to *write* is thrown, as `companion::SaveFormatError` rather than `CompanionError`, since a bad file and an unbalanced companion are different failure categories.
...
`antwika::ecs_commons` is the vocabulary half of the ECS -- `GridPosition`, `Velocity`, `Lifetime`, `Name`, `Tag<Kind>` and the `MovementSystem`/`LifetimeSystem`/`PeriodicSystem` that act on them -- kept out of `antwika::ecs` because that library is the mechanism and these are content, so an app wanting the scheduler does not link a countdown it never uses; see [`wiki/libraries/ecs_commons.md`](wiki/libraries/ecs_commons.md).

`antwika::rng` is the one source of pseudo-random bits, and it depends on nothing but the standard library, in the spirit of `antwika::wfc` -- a caller wanting a deterministic generator does not link a poker engine to get one.
`IRng::next()` returns 64 raw bits and that is the whole interface: there is deliberately no bounded draw, no float and no distribution, since `<random>`'s engines are portable but its distributions are not, and a `std::uniform_int_distribution` that consumed a different number of draws on libc++ than on libstdc++ would be a replay divergence with the symptom nowhere near the cause.
So every bounded draw is written at its call site, in the open, next to the code whose determinism depends on it -- `rng.next() % (index + 1)` in `holdem::Deck`'s Fisher-Yates, `rng.next() % config.height` in `apps/tower_defence`'s level generator -- and the modulo bias that comes with it is stated rather than hidden, since a shuffle and a level layout need reproducibility rather than uniformity to the last bit.
`SplitMix64Rng` is the one production implementation, a fixed sequence of adds, shifts, xors and multiplies over an exact-width `std::uint64_t`, and **its output sequence is part of the contract rather than an implementation detail**: `SplitMix64RngTest` pins the first three draws for seed 0 against the published splitmix64 output rather than against itself, so changing it invalidates every checked-in replay at once, which is the size of alarm the change deserves.
A generator is always injected or a local built from a seed something already persists (`poker::RoomConfig::seed`, `game::SaveGame::seed`, `td::LevelConfig::seed`) -- there is no global generator, no thread-local one, and nothing under `src/` seeded from a clock or a `std::random_device`.
`antwika::wfc` takes no seed at all and is deterministic on its input, so it is not a caller: in `apps/game`'s world map the seed reaches the solver only through which anchor cells are pinned, which is the one place randomness enters.
See [`wiki/libraries/rng.md`](wiki/libraries/rng.md).
...
See [`wiki/libraries/pathfinding.md`](wiki/libraries/pathfinding.md).
...
See [`wiki/libraries/cli.md`](wiki/libraries/cli.md).
...
See [`wiki/libraries/i18n.md`](wiki/libraries/i18n.md).

`antwika::font` turns the bytes of a TrueType font into metrics and coverage masks: `Font`/`TtfReader` parse one, `FontMetrics`/`GlyphMetrics` say what it holds, `rasterise()` draws one glyph into a `Coverage`, and `makeGlyphAtlas()` packs a set of characters into one mask plus the map from a character to the `Rect` holding it.
**The library is named for the domain and its reader for the format**, which is `gfx::PngReader`'s and `sound::WavReader`'s arrangement: it was `antwika::ttf`, and that name promised one file format where metrics and coverage masks are not a TrueType concept at all, so an OpenType/CFF outline path would arrive as a second reader here rather than as a second library -- and renaming is cheapest before anything links it.
`FontError` is read the other way round, naming the library's one failure category as `GfxError` and `SoundError` do.
Nothing about what is refused moved: a font collection and an OpenType font with CFF outlines are still turned away by name, with the same message and the same type.
**It depends on no other module of this project**, which is the decision the rest follows from: a glyph is one channel rather than four, so the output is `font::Coverage` rather than `gfx::Bitmap` -- reusing that struct would have taken on GLM and `antwika::log` for three fields, where expanding a mask into white RGBA with the coverage in alpha is four lines at the one seam that wants a texture, and leaves the tint deciding the colour.
`TtfReader::read()` takes a `std::istream` and `Font`'s constructor takes bytes, for `PngReader`'s and `WavReader`'s two reasons: the library opens no files, and every refusal is therefore reachable from bytes in memory -- which is what makes the coverage gate satisfiable here, since `src/libs/font/tests/SyntheticFont.cpp` builds a whole valid four-glyph font in memory and **no font binary is checked in at all**.
`stb_truetype.h` comes from the `stb` package this project already depends on, so it added no dependency and changed no lockfile; `STBTT_STATIC` is required because raylib links its own copy, and since that also makes every unused entry point a `-Wunused-function` error, `src/StbTrueType.cpp` is compiled with `-w` and therefore holds nothing but one-call wrappers -- every rounding, refusal and packing decision lives in a source the compiler still checks.
`FontDirectory.cpp` checks the offset table and every table record lies inside the blob before the rasteriser sees any of it, because stb trusts the offsets it is handed and a font arrives from a file somebody else wrote; validating a table's *contents* is left to the rasteriser, which reports it by refusing to open the font.
No floating-point value crosses the public interface -- a design unit is scaled and rounded exactly once -- and **nothing this library returns may enter simulation state**, which is `gfx::Transform`'s rule rather than `animation::Progress`'s exactness: an outline is floating point in the file, so a glyph box differing in its last bit costs a pixel and never a divergent replay.
Lookup is total following `antwika::i18n`: an unmapped codepoint rasterises as the font's own `.notdef` rather than throwing, `has()` is how a caller asks in advance, and a blank glyph is an ordinary answer with an empty mask and a non-zero advance.
`antwika::gfx`'s built-in `Glyphs` and `gfx::textSize()` are deliberately *not* wired onto this yet; that is the natural follow-up and a much larger change, since it would alter what every existing `antwika::ui` layout test expects.
See [`wiki/libraries/font.md`](wiki/libraries/font.md).
...
See [`wiki/libraries/sound.md`](wiki/libraries/sound.md).

`antwika::network` carries bytes between hosts through a build-time backend seam exactly as `gfx`, `input` and `sound` have (`INetworkBackend`/`IHost`/`NetworkCapabilities`, `NetworkError`, `makeSelectedNetworkBackend()`), so no code under `src/` names a socket API; it is chosen by `-o network_backend=` and `ANTWIKA_NETWORK_BACKEND`, defaulting to `null` and deliberately not following graphics, for `sound_backend`'s reason -- a build asking for sdl3 windows has said nothing about wanting to open a socket.
It is the **transport half** of multiplayer and nothing more, and it depends on `antwika::log` and nothing else -- not `event`, not `time`, not `simulation` -- which is structural rather than a promise: this library cannot name a `Tick` or an `Event`, so it cannot fold a socket fact into simulation state even by accident.
**The rule the whole design hangs on is that the network reaches a simulation only as `event::Event`s an `ITickEventSource` supplied for a specific tick**, since remote input *is* input in exactly the sense a click is -- external, unpredictable and arriving from outside the process, which is what that seam already models and what a `--record` file already holds.
A networked session that is recorded therefore replays single-player with no socket opened, and three corollaries follow: no wall clock may decide what is *computed* (latency may decide what is drawn, on `game::FrameMeter`'s terms, and nothing else), a peer dropping is a decision every peer must reach identically rather than one each reaches on its own clock, and there is **no thread, no lock and no queue** -- a host does nothing until `pump()` asks it to, on the thread that asked, which is `sound::IDevice`'s decision taken for sound's reasons.
**A peer's name is local to the host that assigned it**: the id A holds for B is not the id B holds for A and neither travels anywhere, exactly as a socket behaves, so an identity a whole session agrees on belongs to the layer above rather than to a transport that has only ever seen one end of a link.
A peer that has gone is not an error either -- `send()` to an id this host no longer holds is a no-op, since a link dropping between reading `peers()` and sending is what a network does -- and `NetworkError` is left for genuine refusals like an oversized payload or a host already open at an endpoint.
**Nobody being there is not an error either**, and that is the one place saying so costs something: a non-blocking connect over a real network cannot know whether anything is listening until several pumps later, so a backend refusing an unreachable endpoint in `connect()` could only do it by blocking inside a tick -- the answer arrives as the peer never appearing in `peers()` instead, and `LoopbackBackend` could answer at once and deliberately does not, since a contract two backends keep differently is one a caller cannot rely on.
**A host reports where it *is* rather than what it was asked for**, since a backend handed port 0 is being asked to pick one -- which is what lets the conformance suite open every host on an ephemeral port and dial whatever each reports.
**`pump()` is where bytes move, in both directions**: `send()` queues and the bytes leave on the *sender's* next pump, because a real socket takes as much as it feels like and leaves the rest, so a send that wrote its payload out on the spot would either block or fail halfway -- and `LoopbackBackend` defers too rather than handing the packet straight over, so a test passing against it passes against a socket.
`NullNetworkBackend` and `LoopbackBackend` live *in* the library rather than under `backends/`, following the `NullSoundBackend` precedent, which is what puts them inside the coverage gate; all that lives under `backends/null/` is the two-line factory.
**`LoopbackBackend` is what makes real multiplayer a unit test** -- every host in one process with no socket, no port and no wall-clock time spent -- and its `DeliverySchedule` makes latency and loss **scripted rather than sampled**, so a test that drops the second packet drops it on every machine and every toolchain where one drawing from a generator would be a different test every run.
...
`backends/sockets` is the one that leaves the process -- non-blocking TCP over IPv4, chosen with `-o network_backend=sockets` -- and it is **the first backend in this tree that is not a framework**, which is why it is exempt from the one-real-framework rule the other three subsystems obey: both reasons that rule gives are about a framework (a process-global event queue, a doubled dependency graph) and a backend naming the operating system's own socket API has neither, so `-o gfx_backend=sdl3 -o network_backend=sockets` is an ordinary configuration that resolves against `conan-sdl3.lock` unchanged and adds no package at all.
The exemption is written in both places that enforce the rule, `conanfile.py` and `backends/CMakeLists.txt`.
TCP rather than UDP because the layer above is lockstep and needs every peer's input for a tick in order, and what TCP costs in exchange is message boundaries -- so a payload travels behind a four-byte **big-endian** length, big-endian because a wire is one place a byte order is stated rather than inherited from whichever machine wrote it.
`backends/sockets/src/SocketApi.hpp` is the one file naming either platform's socket API, since the two disagree about the handle type, closing one, making one non-blocking and where the last error lives -- and about nothing else this backend does; Winsock's process-wide start-up lives there too as a function-local static, following `backends/sdl3`'s runtime archive.
The shared `antwika::network::tests::conformance` suite runs against all three, and a backend that cannot do something skips on `NetworkCapabilities` rather than failing -- a backend pretending to connect so a test goes green is exactly the dishonesty a conformance suite exists to prevent.
**Writing the second backend is what found the first one's contract gaps**, which is the argument for that suite rather than a story about it: `connect()` refusing an unreachable endpoint and `endpoint()` returning the requested port were both promises only an in-process backend could keep, and the suite pumping one end of a link had been passing only because `LoopbackBackend` delivered on `send()` where a socket cannot.
...
See [`wiki/libraries/network.md`](wiki/libraries/network.md).
...
`antwika::gfx` is nevertheless *on* that link line, transitively: `antwika::simulation` links `antwika::ecs` and `antwika::gfx` for `TickPacer` and `WindowInputSource`, and `antwika::input` links it because every source it offers is an `ITickEventSource`.
...
Live input reaches the engine only through `ITickEventSource`, via `LiveInputSource`, and is persisted by `InputEventCodec` as `input.*` events with symbolic key and button names rather than platform scancodes, so a session recorded under one backend replays under another.
**What lands in a recording is decided by where the recorder sits rather than by a list of names it skips.**
`event::TickEventRecorder` is an ordinary `ITickEventSink` an app registers on its `TickedEventDispatcher`, and it records unconditionally, so what it sees is exactly what an `ITickEventSource` supplied for that tick -- never `engine.tick`, which `Engine::step()` dispatches for itself after `EngineLoop` has drained the source, and never anything a sink derives further down the tick path, like the tile `game::GridSink` lays from a click.
...
**A `antwika::ui` hover is now the general answer to the gate's cost rather than a per-app one**: `app::hoverFrom()` reads a `std::optional<PointerHint>` as a `ui::HoverPointer`, which is the type `ui::applyHover()` takes and the only thing that reaches a picture, so any app drawing a UI gets a hover on approach without one byte entering a recording -- see `docs/hover-is-not-simulation.md`.
...
**A `gfx::Color` is straight alpha and every drawing call blends with it**, `clear()` alone excepted, since that one replaces the drawable area rather than drawing over it -- and what a framework does by default is not the answer, because SDL's renderer starts at `SDL_BLENDMODE_NONE` and has to be asked, where raylib blends already, which is exactly how one backend came to paint a tower's translucent range as a flat opaque blue.
...
**Hover is the fourth answer, and it is the one that leaves the tick path**: `Frame::hoverTargets` is a `ui::HoverTargets`, one `ui::HoverTarget` per named widget that works its own appearance out, and `ui::applyHover(commands, targets, hover)` repaints the picture from a `ui::HoverPointer`.
The framing is the repository owner's and it is the right one -- hover and other pointer effects are **by definition not part of the simulation**, so they may decide what is drawn and nothing else -- and three decisions make that structural rather than a promise.
`ui::HoverPointer` carries a position and *no* `down` and *no* `pressed`, so it cannot say a press happened; `applyHover()` is handed a `DrawList &` and two read-only values and never a `Frame`, so `Interactions` is not reachable from inside it at any cost; and the position it draws from is a value cell (`input::PointerHintChannel`) rather than a marked event, for the reason that channel already gives at length.
Activation, focus, edits and chosen options keep resolving in `detail::resolve()` from the recorded `ui::Pointer` inside the tick path, exactly as before, and `ContextHoverTest` pins that a frame with all four in play has identical `Interactions` before and after a hover pass.
...
`apps/gfx_demo` is the worked example -- it has no recorder, so `DemoLoop` gates the position out of its `ui::Pointer` by hand and publishes the free one to a `PointerHintChannel`, which is what makes the demonstration honest rather than circular -- and `docs/hover-is-not-simulation.md` is the rule written down.
`game::UiSink`/`UiOverlay`/`Toolbar` should adopt it next: that app already owns a hint channel and already draws its placement ghost from it, so the toolbar buttons lighting up on approach is `main.cpp` handing `RenderSystem` the channel and one `applyHover()` call after the sink has resolved the press -- and it is the one remaining thing `apps/game` says it does not do.
