...
- `apps/game` is an isometric grid you build on with the mouse: left-click lays a path tile, left-drag lays a whole run of them, right-click drops a walker onto one, middle-drag pans and the wheel zooms, and F10 opens a menu modal over the city.
  ...
  **The frames drawn between two ticks carry on as well, which is why the pause reaches the picture too**: `game::SceneSnapshot` carries it, read off `PauseState` by `snapshotOf()` exactly as the camera is, and `game::GridScene` then draws a held walker at its step's own phase whatever fraction of a tick a frame falls at.
  ...
  The decision is the scene's rather than whoever supplies the fraction, so a held snapshot is the same picture wherever it is drawn, and `PauseState` is owned by `main.cpp` beside the camera and the mode for the reason all three are: a renderer built before the run has to read it.
